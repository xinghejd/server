...
{
    const string _organizationTwoFactor =
        """{"6":{"Enabled":true,"MetaData":{"ClientId":"DIEFB13LB49IEB3459N2","ClientSecret":"0ZnsZHav0KcNPBZTS6EOUwqLPoB0sfMd5aJeWExQ","Host":"api-example.duosecurity.com"}}}""";
    const string _testEmail = "test+2farequired@email.com";
    const string _testPassword = "master_password_hash";
    const string _userEmailTwoFactor =
        """{"1": { "Enabled": true, "MetaData": { "Email": "test+2farequired@email.com"}}}""";
    ...
    public async Task TokenEndpoint_GrantTypePassword_UserTwoFactorRequired_TwoFactorProvided_Success()
    {
        ...
        {
            mailService
                .SendTwoFactorEmailAsync(Arg.Any<string>(), Arg.Do<string>(t => emailToken = t))
    ...
    [Theory, BitAutoData]
    public async Task TokenEndpoint_GrantTypePassword_OrgDuoTwoFactorRequired_NoTwoFactorProvided_Fails(
        string deviceId
    )
    {
        // Arrange
        ...
        await CreateSsoOrganizationAndUserAsync(
            ...
            challenge,
            _testEmail,
            orgTwoFactor: _organizationTwoFactor
        );
    ...
    [Theory, BitAutoData]
    public async Task TokenEndpoint_GrantTypeClientCredential_OrgTwoFactorRequired_Success(
    ...
    [Theory, BitAutoData]
    public async Task TokenEndpoint_GrantTypeAuthCode_OrgTwoFactorRequired_IndvTwoFactor_NoTwoFactorProvided_Fails(
        string deviceId
    )
    {
        // Arrange
        ...
        await CreateSsoOrganizationAndUserAsync(
            ...
            challenge,
            _testEmail,
            userTwoFactor: _userEmailTwoFactor
        );
    ...
    [Theory, BitAutoData]
    public async Task TokenEndpoint_GrantTypeAuthCode_OrgTwoFactorRequired_IndvTwoFactor_TwoFactorProvided_Success(
        string deviceId
    )
    {
        // Arrange
        ...
        // Act
        var failedTokenContext = await localFactory.Server.PostAsync(
            "/connect/token",
            new FormUrlEncodedContent(
                new Dictionary<string, string>
                {
        ...
        var twoFactorProvidedContext = await localFactory.Server.PostAsync(
            "/connect/token",
            new FormUrlEncodedContent(
                new Dictionary<string, string>
                {
    ...
    [Theory, BitAutoData]
    public async Task TokenEndpoint_GrantTypeAuthCode_OrgTwoFactorRequired_OrgDuoTwoFactor_NoTwoFactorProvided_Fails(
        string deviceId
    )
    {
        // Arrange
    ...
    )
    {
        ...
        await factory.RegisterAsync(
            new RegisterRequestModel { Email = testEmail, MasterPasswordHash = _testPassword }
        );
