...
{
    [Theory, BitAutoData]
    public async Task SaveAsync_DefaultIdWithUsers_CreatesCollectionInTheRepository(
    ...
    [Theory, BitAutoData]
    public async Task SaveAsync_DefaultIdWithGroupsAndUsers_CreateCollectionWithGroupsAndUsersInRepository(
        Collection collection,
    ...
    [Theory, BitAutoData]
    public async Task SaveAsync_NonDefaultId_ReplacesCollectionInRepository(
        Collection collection,
        Organization organization,
        [CollectionAccessSelectionCustomize(true)] IEnumerable<CollectionAccessSelection> users,
        SutProvider<CollectionService> sutProvider
    ...
    [Theory, BitAutoData]
    public async Task SaveAsync_NoManageAccess_ThrowsBadRequest(
        Collection collection,
        Organization organization,
    ...
    )
    {
        ...
        var ex = await Assert.ThrowsAsync<BadRequestException>(
            () => sutProvider.Sut.SaveAsync(collection, null, users)
        );
    ...
    )
    {
        ...
        Assert.Equal(
            $@"You have reached the maximum number of collections ({organization.MaxCollections.Value}) for this organization.",
            ex.Message
        );
        await sutProvider.GetDependency<ICollectionRepository>().DidNotReceiveWithAnyArgs().CreateAsync(default);
        await sutProvider
    ...
    )
    {
        ...
        // invalid user
        await Assert.ThrowsAsync<NotFoundException>(
            () => sutProvider.Sut.DeleteUserAsync(collection, Guid.NewGuid())
        );
        await sutProvider
            .GetDependency<ICollectionRepository>()
            .DidNotReceiveWithAnyArgs()
    ...
    {
        await Assert.ThrowsAsync<NotFoundException>(
            () => sutProvider.Sut.GetOrganizationCollectionsAsync(organizationId)
        );
