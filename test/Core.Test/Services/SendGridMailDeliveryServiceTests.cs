...
public class SendGridMailDeliveryServiceTests : IDisposable
{
    ...
    {
        _globalSettings = new GlobalSettings { Mail = { SendGridApiKey = "SendGridApiKey" } };
        ...
        _sut = new SendGridMailDeliveryService(_sendGridClient, _globalSettings, _hostingEnvironment, _logger);
    }
    ...
    public async Task SendEmailAsync_CallsSendEmailAsync_WhenMessageIsValid()
    {
        ...
        await _sendGridClient
            ...
            .SendEmailAsync(
                ...
                {
                    msg.Received(1)
                        .AddTos(new List<EmailAddress> { new EmailAddress(mailMessage.ToEmails.First()) });
                    msg.Received(1)
                        .AddBccs(new List<EmailAddress> { new EmailAddress(mailMessage.ToEmails.First()) });
