...
public class SendServiceTests
{
    ...
    )
    {
        ...
        var sendOptions = new SendOptionsPolicyData { DisableHideEmail = disableHideEmailAppliesToUser };
    ...
    [Theory]
    [BitAutoData]
    public async Task SaveSendAsync_ExistingSend_Updates(SutProvider<SendService> sutProvider, Send send)
    {
        ...
        await sutProvider.GetDependency<IPushNotificationService>().Received(1).PushSyncSendUpdateAsync(send);
    }

    [Theory]
    [BitAutoData]
    public async Task SaveFileSendAsync_TextType_ThrowsBadRequest(SutProvider<SendService> sutProvider, Send send)
    {
    ...
    [Theory]
    [BitAutoData]
    public async Task SaveFileSendAsync_EmptyFile_ThrowsBadRequest(SutProvider<SendService> sutProvider, Send send)
    {
    ...
    [Theory]
    [BitAutoData]
    public async Task SaveFileSendAsync_HasEnoughStorage_Success(SutProvider<SendService> sutProvider, Send send)
    {
        var user = new User
        {
            Id = Guid.NewGuid(),
    ...
    [Theory]
    [BitAutoData]
    public async Task UpdateFileToExistingSendAsync_Success(SutProvider<SendService> sutProvider, Send send)
    {
    ...
    [Theory]
    [BitAutoData]
    public async Task UpdateFileToExistingSendAsync_InvalidSize(SutProvider<SendService> sutProvider, Send send)
    {
        var fileContents = "Test file content";
    ...
    public void SendCanBeAccessed_Success(SutProvider<SendService> sutProvider, Send send)
    {
        ...
        var (grant, passwordRequiredError, passwordInvalidError) = sutProvider.Sut.SendCanBeAccessed(send, "TEST");
    ...
    [Theory]
    [BitAutoData]
    public void SendCanBeAccessed_NullMaxAccess_Success(SutProvider<SendService> sutProvider, Send send)
    {
        var now = DateTime.UtcNow;
    ...
    public void SendCanBeAccessed_NullSend_DoesNotGrantAccess(SutProvider<SendService> sutProvider)
    {
        ...
        var (grant, passwordRequiredError, passwordInvalidError) = sutProvider.Sut.SendCanBeAccessed(null, "TEST");
    ...
    )
    {
        ...
        var (grant, passwordRequiredError, passwordInvalidError) = sutProvider.Sut.SendCanBeAccessed(send, null);
    ...
    [Theory]
    [BitAutoData]
    public void SendCanBeAccessed_RehashNeeded_RehashesPassword(SutProvider<SendService> sutProvider, Send send)
    {
        var now = DateTime.UtcNow;
        send.MaxAccessCount = null;
        send.AccessCount = 5;
        ...
        sutProvider.GetDependency<IPasswordHasher<User>>().Received(1).HashPassword(Arg.Any<User>(), "TEST");
