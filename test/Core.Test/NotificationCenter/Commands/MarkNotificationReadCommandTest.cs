...
public class MarkNotificationReadCommandTest
{
    ...
    )
    {
        ...
        sutProvider.GetDependency<INotificationRepository>().GetByIdAsync(notificationId).Returns(notification);
        sutProvider
        ...
        sutProvider.GetDependency<INotificationStatusRepository>().CreateAsync(Arg.Any<NotificationStatus>());
        sutProvider
            .GetDependency<INotificationStatusRepository>()
        ...
        await sutProvider
            ...
            .UpdateAsync(
                Arg.Is<NotificationStatus>(ns =>
                    ...
                    && ns.DeletedDate == notificationStatus.DeletedDate
                    && ns.ReadDate != readDate
                    && ns.ReadDate.HasValue
                    && DateTime.UtcNow - ns.ReadDate.Value < TimeSpan.FromMinutes(1)
                )
            );
