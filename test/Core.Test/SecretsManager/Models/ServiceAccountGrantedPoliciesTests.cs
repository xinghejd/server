    ...
    public void GetPolicyUpdates_NoChanges_ReturnsEmptyLists()
    {
        ...
        var existing = new ServiceAccountGrantedPolicies
        {
            ProjectGrantedPolicies = new List<ServiceAccountProjectAccessPolicy>
            {
                new()
                {
                    GrantedProjectId = projectId1,
                    Read = true,
                    Write = true,
                },
                new()
                {
                    GrantedProjectId = projectId2,
                    Read = false,
                    Write = true,
                },
            },
    ...
    public void GetPolicyUpdates_ReturnsCorrectPolicyChanges()
    {
        ...
        var existing = new ServiceAccountGrantedPolicies
        {
            ProjectGrantedPolicies = new List<ServiceAccountProjectAccessPolicy>
            {
                ...
                new()
                {
                    GrantedProjectId = projectId3,
                    Read = true,
                    Write = true,
                },
                new()
                {
                    GrantedProjectId = projectId4,
                    Read = true,
                    Write = true,
                },
            },
        ...
        Assert.Contains(
            projectId2,
            result
                .ProjectGrantedPolicyUpdates.Where(pu => pu.Operation == AccessPolicyOperation.Create)
        ...
        Assert.Contains(
            projectId1,
            result
                .ProjectGrantedPolicyUpdates.Where(pu => pu.Operation == AccessPolicyOperation.Update)
                .Select(pu => pu.AccessPolicy.GrantedProjectId!.Value)
        );

        Assert.DoesNotContain(
            projectId3,
            result.ProjectGrantedPolicyUpdates.Select(pu => pu.AccessPolicy.GrantedProjectId!.Value)
        );
    }
}
