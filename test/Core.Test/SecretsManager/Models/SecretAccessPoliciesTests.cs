...
public class SecretAccessPoliciesTests
{
    ...
    public void GetPolicyUpdates_ReturnsCorrectPolicyChanges()
    {
        ...
        Assert.Contains(
            createId,
            result
                .ServiceAccountAccessPolicyUpdates.Where(pu => pu.Operation == AccessPolicyOperation.Create)
        ...
        Assert.Contains(
            updatedId,
            result
                .ServiceAccountAccessPolicyUpdates.Where(pu => pu.Operation == AccessPolicyOperation.Update)
                .Select(pu => pu.AccessPolicy.ServiceAccountId!.Value)
        );
        ...
        Assert.DoesNotContain(
            unChangedId,
            result.ServiceAccountAccessPolicyUpdates.Select(pu => pu.AccessPolicy.ServiceAccountId!.Value)
        );
    }
}
