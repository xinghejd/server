...
public class AcceptOrgUserCommandTests
{
    ...
    private readonly IOrgUserInviteTokenableFactory _orgUserInviteTokenableFactory =
        Substitute.For<IOrgUserInviteTokenableFactory>();
        ...
        await sutProvider
            ...
            .ReplaceAsync(
                Arg.Is<OrganizationUser>(ou =>
                    ou.Id == orgUser.Id && ou.Status == OrganizationUserStatusType.Accepted
                )
            );
    ...
    )
    {
        ...
        // Act
        var resultOrgUser = await sutProvider.Sut.AcceptOrgUserByEmailTokenAsync(
            orgUser.Id,
            user,
            oldToken,
            _userService
    ...
    )
    {
        ...
        _orgUserInviteTokenableFactory
            .CreateToken(orgUser)
            .Returns(
                new OrgUserInviteTokenable(orgUser) { ExpirationDate = DateTime.UtcNow.Add(TimeSpan.FromDays(5)) }
            );
    ...
    )
    {
        ...
        _orgUserInviteTokenableFactory
            .CreateToken(orgUser)
            .Returns(
                new OrgUserInviteTokenable(orgUser) { ExpirationDate = DateTime.UtcNow.Add(TimeSpan.FromDays(-1)) }
            );
    ...
    [BitAutoData(OrganizationUserStatusType.Confirmed, "You are already part of this organization.")]
    ...
    )
    {
        ...
        sutProvider.GetDependency<IOrganizationRepository>().GetByIdentifierAsync(org.Identifier).Returns(org);
    ...
    [Theory]
    [BitAutoData]
    public async Task AcceptOrgUserByOrgSsoIdAsync_InvalidOrg_ThrowsBadRequest(
        SutProvider<AcceptOrgUserCommand> sutProvider,
    ...
    [Theory]
    [BitAutoData]
    public async Task AcceptOrgUserByOrgSsoIdAsync_UserNotInOrg_ThrowsBadRequest(
        SutProvider<AcceptOrgUserCommand> sutProvider,
        Organization org,
        User user
    )
    ...
    [Theory]
    [BitAutoData]
    public async Task AcceptOrgUserByOrgId_InvalidOrg_ThrowsBadRequest(
        SutProvider<AcceptOrgUserCommand> sutProvider,
        Guid orgId,
        User user
    )
    {
        ...
        sutProvider.GetDependency<IOrganizationRepository>().GetByIdAsync(orgId).Returns((Organization)null);
    ...
    [Theory]
    [BitAutoData]
    public async Task AcceptOrgUserByOrgId_UserNotInOrg_ThrowsBadRequest(
        SutProvider<AcceptOrgUserCommand> sutProvider,
        Organization org,
        User user
    )
    ...
    /// </summary>
    private void AssertValidAcceptedOrgUser(
        OrganizationUser resultOrgUser,
    ...
    private void SetupCommonAcceptOrgUserByTokenMocks(
        SutProvider<AcceptOrgUserCommand> sutProvider,
        User user,
        OrganizationUser orgUser
    )
    ...
    /// </summary>
    private void SetupCommonAcceptOrgUserMocks(
        SutProvider<AcceptOrgUserCommand> sutProvider,
        User user,
        Organization org,
        OrganizationUser orgUser,
    ...
    )
    {
        ...
        sutProvider
            .GetDependency<IOrganizationUserRepository>()
            .GetManyByUserAsync(user.Id)
            .Returns(Task.FromResult<ICollection<OrganizationUser>>(new List<OrganizationUser>()));
        ...
        sutProvider
            ...
            .Returns(
                Task.FromResult<IEnumerable<OrganizationUserUserDetails>>(
                    new List<OrganizationUserUserDetails>() { adminUserDetails }
                )
            );

        // Return org
        sutProvider.GetDependency<IOrganizationRepository>().GetByIdAsync(org.Id).Returns(Task.FromResult(org));
    }

    private string CreateOldToken(SutProvider<AcceptOrgUserCommand> sutProvider, OrganizationUser organizationUser)
    {
        var dataProtector = sutProvider
