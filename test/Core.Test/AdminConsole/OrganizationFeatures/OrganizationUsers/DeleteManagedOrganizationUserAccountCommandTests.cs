...
public class DeleteManagedOrganizationUserAccountCommandTests
{
    ...
    public async Task DeleteUserAsync_WithValidUser_DeletesUserAndLogsEvent(
        ...
        User user,
        Guid deletingUserId,
        [OrganizationUser(OrganizationUserStatusType.Confirmed, OrganizationUserType.User)]
    ...
    )
    {
        ...
        // Act
        await sutProvider.Sut.DeleteUserAsync(
            organizationUser.OrganizationId,
            organizationUser.Id,
            deletingUserId
        );
    ...
    )
    {
        ...
        var exception = await Assert.ThrowsAsync<BadRequestException>(
            () =>
                sutProvider.Sut.DeleteUserAsync(
                    organizationUser.OrganizationId,
                    organizationUser.Id,
                    deletingUserId
                )
    ...
    public async Task DeleteUserAsync_WhenUserIsInvited_ThrowsException(
        SutProvider<DeleteManagedOrganizationUserAccountCommand> sutProvider,
        [OrganizationUser(OrganizationUserStatusType.Invited, OrganizationUserType.User)]
            OrganizationUser organizationUser
    )
    {
        // Arrange
        ...
        await sutProvider
            ...
            .DeleteManyAsync(
                Arg.Is<IEnumerable<User>>(users =>
                    users.Any(u => u.Id == user1.Id) && users.Any(u => u.Id == user2.Id)
                )
            );
        await sutProvider
            .GetDependency<IEventService>()
            ...
            .LogOrganizationUserEventsAsync(
                Arg.Is<IEnumerable<(OrganizationUser, EventType, DateTime?)>>(events =>
                    events.Count(e => e.Item1.Id == orgUser1.Id && e.Item2 == EventType.OrganizationUser_Deleted)
                        == 1
                    && events.Count(e =>
                        e.Item1.Id == orgUser2.Id && e.Item2 == EventType.OrganizationUser_Deleted
                    ) == 1
                )
            );
    }
    ...
    )
    {
        ...
        // Act
        var result = await sutProvider.Sut.DeleteManyUsersAsync(
    ...
    )
    {
        ...
        sutProvider.GetDependency<ICurrentContext>().OrganizationOwner(orgUser.OrganizationId).Returns(false);
    ...
    )
    {
        ...
        sutProvider.GetDependency<ICurrentContext>().OrganizationOwner(orgUser.OrganizationId).Returns(true);
    ...
    )
    {
        ...
        // Act
        var results = await sutProvider.Sut.DeleteManyUsersAsync(
            organizationId,
            new[] { orgUser1.Id, orgUser2.Id, orgUser3.Id },
            null
        );
        ...
        Assert.Equal(
            "You cannot delete a member with Invited status.",
            results.First(r => r.Item1 == orgUser2.Id).Item2
        );
        Assert.Equal(
            "Member is not managed by the organization.",
            results.First(r => r.Item1 == orgUser3.Id).Item2
        );
