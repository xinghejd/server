...
{
    [Theory, BitAutoData]
    public async Task UserVerifyOrganizationDomainAsync_ShouldThrowConflict_WhenDomainHasBeenClaimed(
        Guid id,
    ...
    [Theory, BitAutoData]
    public async Task UserVerifyOrganizationDomainAsync_ShouldThrowConflict_WhenDomainHasBeenClaimedByAnotherOrganization(
        Guid id,
        SutProvider<VerifyOrganizationDomainCommand> sutProvider
    )
    {
    ...
    [Theory, BitAutoData]
    public async Task UserVerifyOrganizationDomainAsync_ShouldVerifyDomainUpdateAndLogEvent_WhenTxtRecordExists(
        Guid id,
        SutProvider<VerifyOrganizationDomainCommand> sutProvider
    )
    {
        ...
        sutProvider
            .GetDependency<IDnsResolverService>()
    ...
    [Theory, BitAutoData]
    public async Task UserVerifyOrganizationDomainAsync_ShouldNotSetVerifiedDate_WhenTxtRecordDoesNotExist(
        Guid id,
        SutProvider<VerifyOrganizationDomainCommand> sutProvider
    )
    {
    ...
    [Theory, BitAutoData]
    public async Task SystemVerifyOrganizationDomainAsync_CallsEventServiceWithUpdatedJobRunCount(
        SutProvider<VerifyOrganizationDomainCommand> sutProvider
    )
    {
    ...
    )
    {
        ...
        sutProvider.GetDependency<IDnsResolverService>().ResolveAsync(domain.DomainName, domain.Txt).Returns(true);
        ...
        await sutProvider
            ...
            .Received(1)
            .SaveAsync(
                Arg.Is<PolicyUpdate>(x =>
                    x.Type == PolicyType.SingleOrg
    ...
    )
    {
        ...
        await sutProvider.GetDependency<ISavePolicyCommand>().DidNotReceive().SaveAsync(Arg.Any<PolicyUpdate>());
    }
    ...
    )
    {
        ...
        var mockedUsers = organizationUsers
            .Where(x =>
                x.Status != OrganizationUserStatusType.Invited && x.Status != OrganizationUserStatusType.Revoked
            )
            .ToList();
        ...
        await sutProvider
            ...
            .Received()
            .SendClaimedDomainUserEmailAsync(
