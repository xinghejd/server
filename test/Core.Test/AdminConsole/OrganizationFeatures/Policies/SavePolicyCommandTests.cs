...
public class SavePolicyCommandTests
{
    ...
    public async Task SaveAsync_NewPolicy_Success([PolicyUpdate(PolicyType.SingleOrg)] PolicyUpdate policyUpdate)
    {
        ...
        await sutProvider
            .GetDependency<IPolicyRepository>()
            .Received(1)
            .UpsertAsync(Arg.Is<Policy>(p => p.CreationDate == creationDate && p.RevisionDate == creationDate));
    }
    ...
    public async Task SaveAsync_OrganizationDoesNotExist_ThrowsBadRequest(
        [PolicyUpdate(PolicyType.ActivateAutofill)] PolicyUpdate policyUpdate
    )
    {
    ...
    )
    {
        ...
        sutProvider
            .GetDependency<IApplicationCacheService>()
            .GetOrganizationAbilityAsync(policyUpdate.OrganizationId)
            .Returns(new OrganizationAbility { Id = policyUpdate.OrganizationId, UsePolicies = false });
    ...
    {
        var sutProvider = SutProviderFactory(
            [new FakeRequireSsoPolicyValidator(), new FakeSingleOrgPolicyValidator()]
        );
        ...
        Assert.Contains(
            "Turn on the Single organization policy because it is required for the Require single sign-on authentication policy",
    ...
    ) // depends on Single Org
    {
        ...
        Assert.Contains(
            "Turn off the Require single sign-on authentication policy because it requires the Single organization policy",
            badRequestException.Message,
            StringComparison.OrdinalIgnoreCase
        );
        await AssertPolicyNotSavedAsync(sutProvider);
    ...
    ) // depends on Single Org
    {
        ...
        Assert.Contains(
            "Turn off all of the policies that require the Single organization policy",
            badRequestException.Message,
            StringComparison.OrdinalIgnoreCase
        );
        await AssertPolicyNotSavedAsync(sutProvider);
    ...
    ) // depends on Single Org but is not enabled
    {
        var sutProvider = SutProviderFactory(
            [new FakeRequireSsoPolicyValidator(), new FakeSingleOrgPolicyValidator()]
        );
    ...
    private static SutProvider<SavePolicyCommand> SutProviderFactory(
        IEnumerable<IPolicyValidator>? policyValidators = null
    )
    {
    ...
    private static void ArrangeOrganization(SutProvider<SavePolicyCommand> sutProvider, PolicyUpdate policyUpdate)
    {
        sutProvider
            .GetDependency<IApplicationCacheService>()
            .GetOrganizationAbilityAsync(policyUpdate.OrganizationId)
            .Returns(new OrganizationAbility { Id = policyUpdate.OrganizationId, UsePolicies = true });
    }

    private static async Task AssertPolicyNotSavedAsync(SutProvider<SavePolicyCommand> sutProvider)
    {
        await sutProvider.GetDependency<IPolicyRepository>().DidNotReceiveWithAnyArgs().UpsertAsync(default!);
    ...
    private static async Task AssertPolicySavedAsync(
        SutProvider<SavePolicyCommand> sutProvider,
