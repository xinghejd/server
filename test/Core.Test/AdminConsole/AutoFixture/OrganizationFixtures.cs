...
public class OrganizationCustomization : ICustomization
{
    ...
    public void Customize(IFixture fixture)
    {
        ...
        var seats = (short)
            new Random().Next(plan.PasswordManager.BaseSeats, plan.PasswordManager.MaxSeats ?? short.MaxValue);
        var smSeats = plan.SupportsSecretsManager
            ? (short?)
                new Random().Next(plan.SecretsManager.BaseSeats, plan.SecretsManager.MaxSeats ?? short.MaxValue)
            : null;
...
internal class PaidOrganization : ICustomization
{
    ...
    public void Customize(IFixture fixture)
    {
        var validUpgradePlans = StaticStore
            .Plans.Where(p => p.Type != PlanType.Free && p.LegacyYear == null)
            .OrderBy(p => p.UpgradeSortOrder)
            .Select(p => p.Type)
            .ToList();
        ...
        fixture.Customize<Organization>(composer => composer.With(o => o.PlanType, CheckedPlanType));
        fixture.Customize<OrganizationUpgrade>(composer =>
            composer.With(ou => ou.Plan, validUpgradePlans.First())
        );
    }
}
...
internal class FreeOrganization : ICustomization
{
    public void Customize(IFixture fixture)
    {
        fixture.Customize<Organization>(composer => composer.With(o => o.PlanType, PlanType.Free));
    }
}
...
internal class FreeOrganizationUpgrade : ICustomization
{
    public void Customize(IFixture fixture)
    {
        ...
        fixture.Customize<Organization>(composer => composer.Without(o => o.GatewaySubscriptionId));
    }
}
...
internal class OrganizationInvite : ICustomization
{
    ...
    public void Customize(IFixture fixture)
    {
        ...
        fixture.Customize<Organization>(composer =>
            composer.With(o => o.Id, organizationId).With(o => o.Seats, (short)100)
        );
        ...
        fixture.Customize<OrganizationUserInvite>(composer => composer.With(oi => oi.Type, InviteeUserType));
        // Set Manage to false, this ensures it doesn't conflict with the other properties during validation
        fixture.Customize<CollectionAccessSelection>(composer => composer.With(c => c.Manage, false));
    }
}
...
public class OrganizationCustomizeAttribute : BitCustomizeAttribute
{
    ...
    public override ICustomization GetCustomization() =>
        new OrganizationCustomization() { UseGroups = UseGroups, PlanType = PlanType };
}
...
internal class PaidOrganizationCustomizeAttribute : BitCustomizeAttribute
{
    ...
    public override ICustomization GetCustomization() =>
        new PaidOrganization() { CheckedPlanType = CheckedPlanType };
}
...
{
    public override ICustomization GetCustomization() => new SecretsManagerOrganizationCustomization();
}
...
{
    public EphemeralDataProtectionAutoDataAttribute()
        : base(new SutProviderCustomization(), new EphemeralDataProtectionCustomization()) { }
}
