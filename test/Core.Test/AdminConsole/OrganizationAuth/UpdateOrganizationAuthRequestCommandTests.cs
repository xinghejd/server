...
public class UpdateOrganizationAuthRequestCommandTests
{
    ...
    )
    {
        ...
        sutProvider
            .GetDependency<IAuthRequestService>()
            .UpdateAuthRequestAsync(
                requestId,
                userId,
        ...
        sutProvider.GetDependency<IUserRepository>().GetByIdAsync(userId).Returns(new User() { Email = email });
        ...
        await sutProvider
            ...
            .Received(1)
            .SendTrustedDeviceAdminApprovalEmailAsync(
                email,
                responseDate,
                requestIpAddress,
                expectedDeviceTypeAndIdentifier
            );
    }
    ...
    )
    {
        sutProvider
            ...
            .GetManyAdminApprovalRequestsByManyIdsAsync(configuration.OrganizationId, updates.Select(ar => ar.Id))
        ...
        await sutProvider
            ...
            .UpdateManyAsync(Arg.Any<IEnumerable<OrganizationAdminAuthRequest>>());
        await sutProvider
            .GetDependency<IPushNotificationService>()
            .DidNotReceiveWithAnyArgs()
            .PushAuthRequestResponseAsync(Arg.Any<AuthRequest>());
        await sutProvider
            .GetDependency<IMailService>()
            .DidNotReceiveWithAnyArgs()
            .SendTrustedDeviceAdminApprovalEmailAsync(
    ...
    )
    {
        ...
        {
            unprocessedAuthRequests[i] = UnrespondAndEnsureValid(
                unprocessedAuthRequests[i],
        ...
        sutProvider
            ...
            .PasswordlessAuth.AdminRequestExpiration.Returns(TimeSpan.FromDays(7));
        ...
        sutProvider
            ...
            .GetManyAsync(
                Arg.Is<IEnumerable<Guid>>(list => list.All(x => organizationUsers.Select(y => y.Id).Contains(x)))
            )
        ...
        foreach (var authRequest in unprocessedAuthRequests)
        {
            await sutProvider
                .GetDependency<IPushNotificationService>()
                .Received()
                .PushAuthRequestResponseAsync(
                    Arg.Is<OrganizationAdminAuthRequest>(ar =>
                        ar.Id == authRequest.Id && ar.Approved == true && ar.Key == "key"
                    )
                );
        ...
        await sutProvider
            ...
            .LogOrganizationUserEventsAsync(
                Arg.Is<IEnumerable<(OrganizationUser o, EventType e, DateTime? d)>>(list =>
                    list.Any()
                    && list.All(x =>
                        organizationUsers.Any(y => y.Id == x.o.Id)
                        && x.e == EventType.OrganizationUser_ApprovedAuthRequest
                    )
                )
            );
    }
    ...
    private T UnrespondAndEnsureValid<T>(T authRequest, Guid organizationId)
        where T : AuthRequest
    {
