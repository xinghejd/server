...
public class OrganizationUsersControllerTests
{
    [Theory]
    [BitAutoData]
    public async Task PutResetPasswordEnrollment_InvitedUser_AcceptsInvite(
        Guid orgId,
        Guid userId,
        OrganizationUserResetPasswordEnrollmentRequestModel model,
        User user,
    ...
    )
    {
        ...
        await sutProvider
            ...
            .Received(1)
            .AcceptOrgUserByOrgIdAsync(orgId, user, sutProvider.GetDependency<IUserService>());
    }

    [Theory]
    [BitAutoData]
    public async Task PutResetPasswordEnrollment_ConfirmedUser_AcceptsInvite(
        Guid orgId,
        Guid userId,
        OrganizationUserResetPasswordEnrollmentRequestModel model,
        User user,
    ...
    )
    {
        ...
        sutProvider
            ...
            .ReturnsForAnyArgs((SsoConfig)null);
        await Assert.ThrowsAsync<BadRequestException>(
            async () => await sutProvider.Sut.PutResetPasswordEnrollment(orgId, userId, model)
        );
    }

    [Theory]
    [BitAutoData]
    public async Task PutResetPasswordEnrollment_PasswordValidationPasses_Continues(
        Guid orgId,
        Guid userId,
        OrganizationUserResetPasswordEnrollmentRequestModel model,
        User user,
    ...
    )
    {
        ...
        await sutProvider
            ...
            .Received(1)
            .UpdateUserResetPasswordEnrollmentAsync(orgId, userId, model.ResetPasswordKey, user.Id);
    }

    [Theory]
    [BitAutoData]
    public async Task Accept_RequiresKnownUser(
        Guid orgId,
        Guid orgUserId,
        OrganizationUserAcceptRequestModel model,
        SutProvider<OrganizationUsersController> sutProvider
    )
    {
    ...
    [Theory]
    [BitAutoData]
    public async Task Accept_NoMasterPasswordReset(
        Guid orgId,
        Guid orgUserId,
        OrganizationUserAcceptRequestModel model,
        User user,
    ...
    )
    {
        ...
        await sutProvider
            .GetDependency<IAcceptOrgUserCommand>()
            .Received(1)
            .AcceptOrgUserByEmailTokenAsync(
                orgUserId,
                user,
                model.Token,
    ...
    [Theory]
    [BitAutoData]
    public async Task Accept_RequireMasterPasswordReset(
        Guid orgId,
        Guid orgUserId,
        OrganizationUserAcceptRequestModel model,
        User user,
    ...
    public async Task Invite_Success(
        ...
        OrganizationUserInviteRequestModel model,
        Guid userId,
        SutProvider<OrganizationUsersController> sutProvider
    )
    {
        ...
        sutProvider
            ...
            .AuthorizeAsync(
                ...
                Arg.Is<IEnumerable<IAuthorizationRequirement>>(reqs =>
                    reqs.Contains(BulkCollectionOperations.ModifyUserAccess)
                )
            )
        ...
        await sutProvider
            ...
            .Received(1)
            .InviteUsersAsync(
                organizationAbility.Id,
                userId,
                systemUser: null,
                Arg.Is<IEnumerable<(OrganizationUserInvite, string)>>(invites =>
                    invites.Count() == 1
    ...
    )
    {
        ...
        sutProvider.GetDependency<ICurrentContext>().ManageUsers(organizationUser.OrganizationId).Returns(true);
    ...
    )
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(
            async () => await sutProvider.Sut.GetAccountRecoveryDetails(organizationId, bulkRequestModel)
        );
    }
    ...
    )
    {
        ...
        await sutProvider
            .GetDependency<IDeleteManagedOrganizationUserAccountCommand>()
            .Received(1)
    ...
    )
    {
        ...
        sutProvider
            .GetDependency<IOrganizationUserUserDetailsQuery>()
            .GetOrganizationUserUserDetails(Arg.Any<OrganizationUserUserDetailsQueryRequest>())
