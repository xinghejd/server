...
public class OrganizationConnectionsControllerTests
{
    ...
    public void ConnectionEnabled_RequiresBothSelfHostAndCommunications(
        bool selfHosted,
        bool enableCloudCommunication,
    ...
    [Theory]
    [BitAutoData]
    public async Task CreateConnection_CloudBillingSync_RequiresOwnerPermissions(
        SutProvider<OrganizationConnectionsController> sutProvider
    )
    {
        var model = new OrganizationConnectionRequestModel { Type = OrganizationConnectionType.CloudBillingSync };
        var exception = await Assert.ThrowsAsync<BadRequestException>(
            () => sutProvider.Sut.CreateConnection(model)
        );
    ...
    public async Task CreateConnection_OnlyOneConnectionOfEachType(
    ...
    )
    {
        ...
        sutProvider
            ...
            .GetByOrganizationIdTypeAsync(model.OrganizationId, type)
    ...
    )
    {
        ...
        sutProvider.GetDependency<ILicensingService>().VerifyLicense(organizationLicense).Returns(false);
    ...
    )
    {
        ...
        sutProvider
            .GetDependency<ICreateOrganizationConnectionCommand>()
            .CreateAsync<BillingSyncConfig>(default)
        ...
        sutProvider.GetDependency<ILicensingService>().VerifyLicense(organizationLicense).Returns(true);
    ...
    [Theory]
    [BitAutoData]
    public async Task UpdateConnection_RequiresOwnerPermissions(
        SutProvider<OrganizationConnectionsController> sutProvider
    )
    {
        sutProvider
        ...
        var exception = await Assert.ThrowsAsync<BadRequestException>(
            () => sutProvider.Sut.UpdateConnection(default, new OrganizationConnectionRequestModel())
        );
    ...
    public async Task UpdateConnection_BillingSync_OnlyOneConnectionOfEachType(
        OrganizationConnectionType type,
        OrganizationConnection existing1,
        OrganizationConnection existing2,
        BillingSyncConfig config,
        SutProvider<OrganizationConnectionsController> sutProvider
    )
    {
        ...
        orgConnectionRepository
            ...
            .Returns(existing1);
        orgConnectionRepository
            .GetByIdOrganizationIdAsync(existing2.Id, existing2.OrganizationId)
            .Returns(existing2);
        orgConnectionRepository
        ...
        var exception = await Assert.ThrowsAsync<BadRequestException>(
            () => sutProvider.Sut.UpdateConnection(existing1.Id, typedModel)
        );

        Assert.Contains(
            $"The requested organization already has a connection of type {typedModel.Type}. Only one of each connection type may exist per organization.",
            exception.Message
        );
    }
    ...
    public async Task UpdateConnection_Scim_OnlyOneConnectionOfEachType(
        OrganizationConnectionType type,
        OrganizationConnection existing1,
        OrganizationConnection existing2,
        ScimConfig config,
    ...
    [Theory]
    [BitAutoData]
    public async Task UpdateConnection_Success(
    ...
    {
        existing.SetConfig(new BillingSyncConfig { CloudOrganizationId = config.CloudOrganizationId });
        ...
        sutProvider
            ...
            .Returns(new[] { existing });
        sutProvider
            .GetDependency<IUpdateOrganizationConnectionCommand>()
    ...
    [Theory]
    [BitAutoData]
    public async Task UpdateConnection_BillingSyncType_InvalidLicense_ErrorThrows(
        OrganizationConnection existing,
        BillingSyncConfig config,
        OrganizationConnection updated,
        SutProvider<OrganizationConnectionsController> sutProvider
    )
    {
        ...
        var exception = await Assert.ThrowsAsync<BadRequestException>(
            async () => await sutProvider.Sut.UpdateConnection(existing.Id, model)
        );
    ...
    [Theory]
    [BitAutoData]
    public async Task UpdateConnection_DoesNotExist_ThrowsNotFound(
        SutProvider<OrganizationConnectionsController> sutProvider
    )
    {
    ...
    [Theory]
    [BitAutoData]
    public async Task GetConnection_RequiresOwnerPermissions(
        Guid connectionId,
        SutProvider<OrganizationConnectionsController> sutProvider
    )
    {
    ...
    [Theory]
    [BitAutoData]
    public async Task GetConnection_Success(
    ...
    [Theory]
    [BitAutoData]
    public async Task DeleteConnection_NotFound(
        Guid connectionId,
        SutProvider<OrganizationConnectionsController> sutProvider
    )
    {
    ...
    [Theory]
    [BitAutoData]
    public async Task DeleteConnection_RequiresOwnerPermissions(
        OrganizationConnection connection,
        SutProvider<OrganizationConnectionsController> sutProvider
    )
    {
        sutProvider
            .GetDependency<IOrganizationConnectionRepository>()
            .GetByIdAsync(connection.Id)
            .Returns(connection);

        var exception = await Assert.ThrowsAsync<BadRequestException>(
            () => sutProvider.Sut.DeleteConnection(connection.Id)
        );
    ...
    [Theory]
    [BitAutoData]
    public async Task DeleteConnection_Success(
        OrganizationConnection connection,
        SutProvider<OrganizationConnectionsController> sutProvider
    )
    {
    ...
    private static JsonDocument JsonDocumentFromObject<T>(T obj) =>
