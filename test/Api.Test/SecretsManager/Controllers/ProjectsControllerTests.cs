...
public class ProjectsControllerTests
{
    ...
    [Theory]
    [BitAutoData]
    public async Task ListByOrganization_SmAccessDenied_Throws(
    ...
    public async Task ListByOrganization_ReturnsEmptyList(
    ...
    )
    {
        ...
        await sutProvider
            ...
            .GetManyByOrganizationIdAsync(
                Arg.Is(AssertHelper.AssertPropertyEqual(data)),
                Arg.Any<Guid>(),
    ...
    )
    {
        ...
        sutProvider
            ...
            .ReturnsForAnyArgs(
                new List<ProjectPermissionDetails>
                {
                    new()
                    {
                        Project = mockProject,
    ...
    [Theory]
    [BitAutoData]
    public async Task Create_NoAccess_Throws(
        SutProvider<ProjectsController> sutProvider,
        Guid orgId,
    ...
    )
    {
        ...
        sutProvider
            .GetDependency<ICreateProjectCommand>()
            .CreateAsync(default, default, sutProvider.GetDependency<ICurrentContext>().IdentityClientType)
    ...
    [Theory]
    [BitAutoData]
    public async Task Create_AtMaxProjects_Throws(
        SutProvider<ProjectsController> sutProvider,
        Guid orgId,
        ProjectCreateRequestModel data
    )
    ...
    [Theory]
    [BitAutoData]
    public async Task Update_NoAccess_Throws(
        SutProvider<ProjectsController> sutProvider,
        Guid userId,
        ProjectUpdateRequestModel data,
    ...
    )
    {
        sutProvider
            ...
            .AuthorizeAsync(
                Arg.Any<ClaimsPrincipal>(),
                data.ToProject(existingProject.Id),
                Arg.Any<IEnumerable<IAuthorizationRequirement>>()
            )
            .ReturnsForAnyArgs(AuthorizationResult.Failed());
        sutProvider
            .GetDependency<IProjectRepository>()
            .GetByIdAsync(existingProject.Id)
            .ReturnsForAnyArgs(existingProject);
        sutProvider.GetDependency<IUserService>().GetProperUserId(default).ReturnsForAnyArgs(userId);

        var resultProject = data.ToProject(existingProject.Id);
        sutProvider.GetDependency<IUpdateProjectCommand>().UpdateAsync(default).ReturnsForAnyArgs(resultProject);

        await Assert.ThrowsAsync<NotFoundException>(() => sutProvider.Sut.UpdateAsync(existingProject.Id, data));
        await sutProvider
            .GetDependency<IUpdateProjectCommand>()
            .DidNotReceiveWithAnyArgs()
    ...
    )
    {
        ...
        await sutProvider.GetDependency<IUpdateProjectCommand>().Received(1).UpdateAsync(Arg.Any<Project>());
    }
    ...
    )
    {
        ...
        await sutProvider.GetDependency<IProjectRepository>().Received(1).GetByIdAsync(Arg.Is(data));
    }

    [Theory]
    [BitAutoData]
    public async Task Get_UserWithoutPermission_Throws(
        SutProvider<ProjectsController> sutProvider,
        Guid orgId,
        Guid data
    )
    ...
    )
    {
        ...
        sutProvider
            .GetDependency<IProjectRepository>()
            .GetManyWithSecretsByIds(Arg.Is(ids))
            .ReturnsForAnyArgs(new List<Project>());
    ...
    )
    {
        ...
        await sutProvider
            .GetDependency<IDeleteProjectCommand>()
            .Received(1)
