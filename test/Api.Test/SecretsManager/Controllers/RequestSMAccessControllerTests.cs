...
public class RequestSMAccessControllerTests
{
    ...
    )
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(
            () => sutProvider.Sut.RequestSMAccessFromAdmins(new RequestSMAccessRequestModel())
        );
    }
    ...
    )
    {
        ...
        await sutProvider
            .GetDependency<IRequestSMAccessCommand>()
            .Received(1)
    ...
    [Theory]
    [BitAutoData]
    public async Task RequestSMAccessFromAdmins_WhenUserInvalid_ShouldThrowBadRequestException(
        RequestSMAccessRequestModel model,
        SutProvider<RequestSMAccessController> sutProvider
    )
    {
        ...
        await Assert.ThrowsAsync<UnauthorizedAccessException>(
            () => sutProvider.Sut.RequestSMAccessFromAdmins(model)
        );
    }

    [Theory]
    [BitAutoData]
    public async Task RequestSMAccessFromAdmins_WhenOrgInvalid_ShouldThrowNotFoundException(
        RequestSMAccessRequestModel model,
        User user,
        SutProvider<RequestSMAccessController> sutProvider
    )
    ...
    [Theory]
    [BitAutoData]
    public async Task RequestSMAccessFromAdmins_WhenOrgUserInvalid_ShouldThrowNotFoundException(
        RequestSMAccessRequestModel model,
        User user,
        SutProvider<RequestSMAccessController> sutProvider
    )
