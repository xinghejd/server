...
public class CollectionsControllerTests
{
    ...
    public async Task Post_Success(
        ...
        CollectionRequestModel collectionRequest,
        SutProvider<CollectionsController> sutProvider
    )
    {
        Collection ExpectedCollection() =>
            Arg.Is<Collection>(c =>
                c.Name == collectionRequest.Name
                && c.ExternalId == collectionRequest.ExternalId
        ...
        await sutProvider
            .GetDependency<ICollectionService>()
            .Received(1)
            .SaveAsync(
                Arg.Any<Collection>(),
    ...
    [Theory, BitAutoData]
    public async Task Put_WithNoCollectionPermission_ThrowsNotFound(
        Collection collection,
        CollectionRequestModel collectionRequest,
        SutProvider<CollectionsController> sutProvider
    )
    {
        ...
        _ = await Assert.ThrowsAsync<NotFoundException>(
            async () => await sutProvider.Sut.Put(collection.OrganizationId, collection.Id, collectionRequest)
        );
    }

    [Theory, BitAutoData]
    public async Task GetOrganizationCollectionsWithGroups_WithReadAllPermissions_GetsAllCollections(
        Organization organization,
        Guid userId,
        SutProvider<CollectionsController> sutProvider
    )
        ...
        sutProvider
            ...
            .AuthorizeAsync(
                ...
                Arg.Is<IEnumerable<IAuthorizationRequirement>>(requirements =>
                    requirements
                        .Cast<CollectionOperationRequirement>()
                        .All(operation =>
    ...
    )
    {
        ...
        sutProvider
            ...
            .AuthorizeAsync(
                ...
                Arg.Is<IEnumerable<IAuthorizationRequirement>>(requirements =>
                    requirements
                        .Cast<BulkCollectionOperationRequirement>()
                        .All(operation => operation.Name == nameof(BulkCollectionOperations.ReadWithAccess))
                )
            )
            .Returns(AuthorizationResult.Success());
        ...
        sutProvider
            ...
            .AuthorizeAsync(
                ...
                Arg.Is<IEnumerable<IAuthorizationRequirement>>(requirements =>
                    requirements
                        .Cast<BulkCollectionOperationRequirement>()
                        .All(operation => operation.Name == nameof(BulkCollectionOperations.Read))
                )
            )
            .Returns(AuthorizationResult.Success());

        sutProvider.GetDependency<ICollectionRepository>().GetManyByUserIdAsync(userId).Returns(collections);
    ...
    [Theory, BitAutoData]
    public async Task DeleteMany_Success(
        Organization organization,
    ...
    )
    {
        ...
        var model = new CollectionBulkDeleteRequestModel { Ids = new[] { collection1.Id, collection2.Id } };

        var collections = new List<Collection>
        {
            new CollectionDetails { Id = collection1.Id, OrganizationId = orgId },
            new CollectionDetails { Id = collection2.Id, OrganizationId = orgId },
        };
        ...
        // Assert
        await sutProvider
            .GetDependency<IDeleteCollectionCommand>()
            .Received(1)
            .DeleteManyAsync(
                Arg.Is<IEnumerable<Collection>>(coll =>
                    coll.Select(c => c.Id).SequenceEqual(collections.Select(c => c.Id))
                )
            );
    }

    [Theory, BitAutoData]
    public async Task DeleteMany_PermissionDenied_ThrowsNotFound(
        Organization organization,
        Collection collection1,
        Collection collection2,
        SutProvider<CollectionsController> sutProvider
    ...
    [Theory, BitAutoData]
    public async Task PostBulkCollectionAccess_Success(
        User actingUser,
        List<Collection> collections,
        Organization organization,
        SutProvider<CollectionsController> sutProvider
    )
    {
        ...
        var model = new BulkCollectionAccessRequestModel
        {
            ...
            Users = new[]
            {
                new SelectionReadOnlyRequestModel { Id = userId, Manage = true },
            },
            Groups = new[]
            {
                new SelectionReadOnlyRequestModel { Id = groupId, ReadOnly = true },
            },
        };
        ...
        sutProvider.GetDependency<ICurrentContext>().UserId.Returns(actingUser.Id);
        ...
        IEnumerable<Collection> ExpectedCollectionAccess() =>
            Arg.Is<IEnumerable<Collection>>(cols => cols.SequenceEqual(collections));
        ...
        await sutProvider
            .GetDependency<IBulkAddCollectionAccessCommand>()
            .Received()
            .AddAccessAsync(
    ...
    [Theory, BitAutoData]
    public async Task PostBulkCollectionAccess_CollectionsNotFound_Throws(
        User actingUser,
        Organization organization,
        List<Collection> collections,
        SutProvider<CollectionsController> sutProvider
    ...
    [Theory, BitAutoData]
    public async Task PostBulkCollectionAccess_CollectionsBelongToDifferentOrganizations_Throws(
        User actingUser,
        Organization organization,
        List<Collection> collections,
        SutProvider<CollectionsController> sutProvider
    ...
    [Theory, BitAutoData]
    public async Task PostBulkCollectionAccess_AccessDenied_Throws(
        User actingUser,
        List<Collection> collections,
        Organization organization,
        SutProvider<CollectionsController> sutProvider
