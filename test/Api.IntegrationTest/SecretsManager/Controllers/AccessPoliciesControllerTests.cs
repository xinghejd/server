...
public class AccessPoliciesControllerTests : IClassFixture<ApiApplicationFactory>, IAsyncLifetime
{
    ...
    {
        var project = await _projectRepository.CreateAsync(
            new Project { OrganizationId = organizationUser.OrganizationId, Name = _mockEncryptedString }
        );
    ...
    )> SetupServiceAccountPeoplePermissionAsync(PermissionType permissionType, OrganizationUser organizationUser)
    {
        var serviceAccount = await _serviceAccountRepository.CreateAsync(
            new ServiceAccount { OrganizationId = organizationUser.OrganizationId, Name = _mockEncryptedString }
        );
    ...
    )> SetupServiceAccountPeopleRequestAsync(PermissionType permissionType, OrganizationUser organizationUser)
    {
    ...
    )
    {
        ...
        var project = await _projectRepository.CreateAsync(
            new Project { Name = _mockEncryptedString, OrganizationId = organizationUser.OrganizationId }
        );
        var accessPolicies = new List<BaseAccessPolicy>
        {
            new UserProjectAccessPolicy
    ...
    )
    {
        var (project, currentUser) = await SetupProjectPeoplePermissionAsync(permissionType, organizationUser);
        var serviceAccount = await _serviceAccountRepository.CreateAsync(
            new ServiceAccount { Name = _mockEncryptedString, OrganizationId = currentUser.OrganizationId }
        );
