...
public class MockedHttpResponse : IMockedHttpResponse
{
    ...
    {
        return AddChild(
            (_, builder) => headers.Aggregate(builder, (b, header) => b.WithHeader(header.Key, header.Value))
        );
    }
    ...
    {
        return WithContent(
            new ByteArrayContent(content) { Headers = { ContentType = new MediaTypeHeaderValue(mediaType) } }
        );
    }
