...
public class ImportCiphersController : Controller
{
    ...
    public async Task PostImport([FromBody] ImportCiphersRequestModel model)
    {
        if (
            ...
            && (
                model.Ciphers.Count() > 7000
                || model.FolderRelationships.Count() > 7000
    ...
    )
    {
        if (
            ...
            && (
                model.Ciphers.Count() > 7000
                || model.CollectionRelationships.Count() > 14000
    ...
    private async Task<bool> CheckOrgImportPermission(List<Collection> collections, Guid orgId)
    {
        ...
        if (
            !(
                await _authorizationService.AuthorizeAsync(User, collections, BulkCollectionOperations.Create)
            ).Succeeded
        )
        {
            return false;
