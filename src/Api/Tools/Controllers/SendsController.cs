...
public class SendsController : Controller
{
    ...
    public async Task<IActionResult> Access(string id, [FromBody] SendAccessRequestModel model)
    {
        ...
        var (send, passwordRequired, passwordInvalid) = await _sendService.AccessAsync(guid, model.Password);
        if (passwordRequired)
        {
    ...
    )
    {
        ...
        var (url, passwordRequired, passwordInvalid) = await _sendService.GetSendFileDownloadUrlAsync(
            send,
            fileId,
            model.Password
        );
