...
public class OrganizationAuthRequestsController : Controller
{
    ...
    [HttpGet("")]
    public async Task<ListResponseModel<PendingOrganizationAuthRequestResponseModel>> GetPendingRequests(
        Guid orgId
    )
    {
        ...
        var responses = authRequests.Select(a => new PendingOrganizationAuthRequestResponseModel(a)).ToList();
    ...
    )
    {
        ...
        var authRequest = (
            await _authRequestRepository.GetManyAdminApprovalRequestsByManyIdsAsync(orgId, new[] { requestId })
        ).FirstOrDefault();
