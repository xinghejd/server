...
public class OrganizationUsersController : Controller
{
    ...
    public async Task<OrganizationUserDetailsResponseModel> Get(Guid id, bool includeGroups = false)
    {
        ...
        var response = new OrganizationUserDetailsResponseModel(
            organizationUser,
            managedByOrganization[organizationUser.Id],
            collections
        );
    ...
    [HttpGet("")]
    public async Task<ListResponseModel<OrganizationUserUserDetailsResponseModel>> Get(
        Guid orgId,
    ...
    )
    {
        var authorized = (
            await _authorizationService.AuthorizeAsync(
                User,
                new OrganizationScope(orgId),
                OrganizationUserUserDetailsOperations.ReadAll
            )
        ).Succeeded;
        if (!authorized)
        {
        ...
        var organizationUsersManagementStatus = await GetManagedByOrganizationStatusAsync(
            orgId,
            organizationUsers.Select(o => o.Id)
        );
        var responses = organizationUsers.Select(o =>
        {
            var userTwoFactorEnabled = organizationUsersTwoFactorEnabled
                .FirstOrDefault(u => u.user.Id == o.Id)
                .twoFactorIsEnabled;
            var managedByOrganization = organizationUsersManagementStatus[o.Id];
            var orgUser = new OrganizationUserUserDetailsResponseModel(
                o,
    ...
    public async Task<IEnumerable<string>> GetGroups(string orgId, string id)
    {
        ...
        if (
            ...
            || (
                !await _currentContext.ManageGroups(organizationUser.OrganizationId)
    ...
    public async Task<OrganizationUserResetPasswordDetailsResponseModel> GetResetPasswordDetails(
        string orgId,
        string id
    )
    {
        ...
        return new OrganizationUserResetPasswordDetailsResponseModel(
            new OrganizationUserResetPasswordDetails(organizationUser, user, org)
        );
    }

    [HttpPost("account-recovery-details")]
    public async Task<
        ListResponseModel<OrganizationUserResetPasswordDetailsResponseModel>
    > GetAccountRecoveryDetails(Guid orgId, [FromBody] OrganizationUserBulkRequestModel model)
    {
        // Make sure the calling user can reset passwords for this org
        ...
        var responses = await _organizationUserRepository.GetManyAccountRecoveryDetailsByOrganizationUserAsync(
            orgId,
            model.Ids
        );
        return new ListResponseModel<OrganizationUserResetPasswordDetailsResponseModel>(
            responses.Select(r => new OrganizationUserResetPasswordDetailsResponseModel(r))
        );
    }
    ...
    public async Task Invite(Guid orgId, [FromBody] OrganizationUserInviteRequestModel model)
    {
        ...
        await _organizationService.InviteUsersAsync(
            orgId,
            userId.Value,
            systemUser: null,
    ...
    [HttpPost("reinvite")]
    public async Task<ListResponseModel<OrganizationUserBulkResponseModel>> BulkReinvite(
        string orgId,
    ...
    public async Task AcceptInit(
        Guid orgId,
        Guid organizationUserId,
        [FromBody] OrganizationUserAcceptInitRequestModel model
    )
    {
        ...
        await _acceptOrgUserCommand.AcceptOrgUserByEmailTokenAsync(
            organizationUserId,
            user,
            model.Token,
            _userService
    ...
    public async Task Accept(
        Guid orgId,
        Guid organizationUserId,
        [FromBody] OrganizationUserAcceptRequestModel model
    )
    {
        var user = await _userService.GetUserByPrincipalAsync(User);
        if (user == null)
        ...
        var masterPasswordPolicy = await _policyRepository.GetByOrganizationIdTypeAsync(
            orgId,
            PolicyType.ResetPassword
        );
        var useMasterPasswordPolicy =
    ...
    [HttpPost("confirm")]
    public async Task<ListResponseModel<OrganizationUserBulkResponseModel>> BulkConfirm(
        string orgId,
    ...
    )
    {
        ...
        return new ListResponseModel<OrganizationUserBulkResponseModel>(
            results.Select(r => new OrganizationUserBulkResponseModel(r.Item1.Id, r.Item2))
        );
    }

    [HttpPost("public-keys")]
    public async Task<ListResponseModel<OrganizationUserPublicKeyResponseModel>> UserPublicKeys(
        string orgId,
        [FromBody] OrganizationUserBulkRequestModel model
    )
    {
        ...
        var result = await _organizationUserRepository.GetManyPublicKeysByOrganizationUserAsync(
            orgGuidId,
            model.Ids
        );
        var responses = result
            .Select(r => new OrganizationUserPublicKeyResponseModel(r.Id, r.UserId, r.PublicKey))
            .ToList();
    ...
    public async Task Put(Guid orgId, Guid id, [FromBody] OrganizationUserUpdateRequestModel model)
    {
        ...
        var groupsToSave =
            editingSelf && !organizationAbility.AllowAdminAccessToAllCollectionItems ? null : model.Groups;
        ...
        await _updateOrganizationUserCommand.UpdateUserAsync(
            model.ToOrganizationUser(organizationUser),
            userId,
            collectionsToSave,
            groupsToSave
        );
    ...
    )
    {
        ...
        var isTdeEnrollment =
            ssoConfig != null
            && ssoConfig.GetData().MemberDecryptionType == MemberDecryptionType.TrustedDeviceEncryption;
        if (
            !isTdeEnrollment
            && !string.IsNullOrWhiteSpace(model.ResetPasswordKey)
            && !await _userService.VerifySecretAsync(user, model.MasterPasswordHash)
        )
        {
    ...
    public async Task PutResetPassword(
        string orgId,
        string id,
    ...
    )
    {
        ...
        var result = await _userService.AdminResetPasswordAsync(
            orgUserType.Value,
            orgGuidId,
            new Guid(id),
    ...
    [HttpDelete("")]
    [HttpPost("remove")]
    public async Task<ListResponseModel<OrganizationUserBulkResponseModel>> BulkRemove(
        Guid orgId,
        [FromBody] OrganizationUserBulkRequestModel model
    )
    {
        ...
        return new ListResponseModel<OrganizationUserBulkResponseModel>(
            result.Select(r => new OrganizationUserBulkResponseModel(r.OrganizationUserId, r.ErrorMessage))
        );
    }
    ...
    public async Task<ListResponseModel<OrganizationUserBulkResponseModel>> BulkDeleteAccount(
        Guid orgId,
        [FromBody] OrganizationUserBulkRequestModel model
    )
    {
        ...
        var results = await _deleteManagedOrganizationUserAccountCommand.DeleteManyUsersAsync(
            orgId,
            model.Ids,
            currentUser.Id
        );
    ...
    [HttpPatch("revoke")]
    [HttpPut("revoke")]
    public async Task<ListResponseModel<OrganizationUserBulkResponseModel>> BulkRevokeAsync(
        Guid orgId,
        [FromBody] OrganizationUserBulkRequestModel model
    )
    {
    ...
    {
        await RestoreOrRevokeUserAsync(
            orgId,
            id,
            (orgUser, userId) => _organizationService.RestoreUserAsync(orgUser, userId)
        );
    }

    [HttpPatch("restore")]
    [HttpPut("restore")]
    public async Task<ListResponseModel<OrganizationUserBulkResponseModel>> BulkRestoreAsync(
        Guid orgId,
        [FromBody] OrganizationUserBulkRequestModel model
    )
    {
        return await RestoreOrRevokeUsersAsync(
            orgId,
            model,
            (orgId, orgUserIds, restoringUserId) =>
                _organizationService.RestoreUsersAsync(orgId, orgUserIds, restoringUserId, _userService)
        );
    }

    [HttpPatch("enable-secrets-manager")]
    [HttpPut("enable-secrets-manager")]
    public async Task BulkEnableSecretsManagerAsync(Guid orgId, [FromBody] OrganizationUserBulkRequestModel model)
    {
        if (!await _currentContext.ManageUsers(orgId))
        {
            throw new NotFoundException();
        ...
        var orgUsers = (await _organizationUserRepository.GetManyAsync(model.Ids))
            .Where(ou => ou.OrganizationId == orgId && !ou.AccessSecretsManager)
            .ToList();
        if (orgUsers.Count == 0)
        {
    ...
    private async Task<ListResponseModel<OrganizationUserBulkResponseModel>> RestoreOrRevokeUsersAsync(
        ...
        Func<
            ...
            IEnumerable<Guid>,
            Guid?,
            Task<List<Tuple<Core.Entities.OrganizationUser, string>>>
        > statusAction
    )
    {
        if (!await _currentContext.ManageUsers(orgId))
        ...
        return new ListResponseModel<OrganizationUserBulkResponseModel>(
            result.Select(r => new OrganizationUserBulkResponseModel(r.Item1.Id, r.Item2))
        );
    }

    private async Task<IDictionary<Guid, bool>> GetManagedByOrganizationStatusAsync(
        Guid orgId,
    ...
    )
    {
        ...
        var usersOrganizationManagementStatus =
            await _getOrganizationUsersManagementStatusQuery.GetUsersOrganizationManagementStatusAsync(
                orgId,
                userIds
            );
