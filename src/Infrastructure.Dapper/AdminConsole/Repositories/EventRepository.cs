...
public class EventRepository : Repository<Event, Guid>, IEventRepository
{
    ...
    )
    {
        return await GetManyAsync(
            ...
            new Dictionary<string, object?> { ["@ProviderId"] = providerId, ["@ActingUserId"] = actingUserId },
            startDate,
            endDate,
            pageOptions
        );
    ...
    )
    {
        ...
        {
            var events = (
                await connection.QueryAsync<Event>(sprocName, parameters, commandType: CommandType.StoredProcedure)
            ).ToList();
