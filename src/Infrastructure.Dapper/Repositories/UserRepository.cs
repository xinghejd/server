...
public class UserRepository : Repository<User, Guid>, IUserRepository
{
    ...
    public UserRepository(GlobalSettings globalSettings, IDataProtectionProvider dataProtectionProvider)
    ...
    )
    {
        ...
        try
        {
            ...
            await using (var cmd = new SqlCommand("[dbo].[User_UpdateKeys]", connection, transaction))
            {
                ...
                cmd.Parameters.Add("@PrivateKey", SqlDbType.VarChar).Value = string.IsNullOrWhiteSpace(
                    user.PrivateKey
                )
                    ? DBNull.Value
                    : user.PrivateKey;
    ...
    private async Task ProtectDataAndSaveAsync(User user, Func<Task> saveTask)
    {
        ...
        {
            user.Key = string.Concat(Constants.DatabaseFieldProtectedPrefix, _dataProtector.Protect(user.Key!));
        }
    ...
    private void UnprotectData(User? user)
    {
        ...
        if (user.Key?.StartsWith(Constants.DatabaseFieldProtectedPrefix) ?? false)
        {
            user.Key = _dataProtector.Unprotect(user.Key.Substring(Constants.DatabaseFieldProtectedPrefix.Length));
        }
    }
