...
public class StripeEventService : IStripeEventService
{
    ...
    public async Task<Charge> GetCharge(Event stripeEvent, bool fresh = false, List<string> expand = null)
    {
        ...
        {
            _logger.LogWarning(
                "Cannot retrieve up-to-date Charge for Event with ID '{eventId}' because no Charge ID was included in the Event.",
                stripeEvent.Id
            );
            return eventCharge;
        }
    ...
    public async Task<Customer> GetCustomer(Event stripeEvent, bool fresh = false, List<string> expand = null)
    {
        ...
        {
            _logger.LogWarning(
                "Cannot retrieve up-to-date Customer for Event with ID '{eventId}' because no Customer ID was included in the Event.",
                stripeEvent.Id
            );
            return eventCustomer;
        }

        var customer = await _stripeFacade.GetCustomer(
            eventCustomer.Id,
    ...
    public async Task<Invoice> GetInvoice(Event stripeEvent, bool fresh = false, List<string> expand = null)
    {
        ...
        {
            _logger.LogWarning(
                "Cannot retrieve up-to-date Invoice for Event with ID '{eventId}' because no Invoice ID was included in the Event.",
                stripeEvent.Id
            );
            return eventInvoice;
        }
    ...
    )
    {
        ...
        {
            _logger.LogWarning(
                "Cannot retrieve up-to-date Payment Method for Event with ID '{eventId}' because no Payment Method ID was included in the Event.",
                stripeEvent.Id
            );
            return eventPaymentMethod;
        }

        var paymentMethod = await _stripeFacade.GetPaymentMethod(
            eventPaymentMethod.Id,
            new PaymentMethodGetOptions { Expand = expand }
        );
    ...
    )
    {
        ...
        {
            _logger.LogWarning(
                "Cannot retrieve up-to-date Subscription for Event with ID '{eventId}' because no Subscription ID was included in the Event.",
                stripeEvent.Id
            );
            return eventSubscription;
        }

        var subscription = await _stripeFacade.GetSubscription(
            eventSubscription.Id,
            new SubscriptionGetOptions { Expand = expand }
        );
    ...
    public async Task<bool> ValidateCloudRegion(Event stripeEvent)
    {
        ...
        var customerMetadata = stripeEvent.Type switch
        {
            HandledStripeWebhook.SubscriptionDeleted or HandledStripeWebhook.SubscriptionUpdated => (
                await GetSubscription(stripeEvent, true, customerExpansion)
            )
                ?.Customer
                ?.Metadata,

            HandledStripeWebhook.ChargeSucceeded or HandledStripeWebhook.ChargeRefunded => (
                await GetCharge(stripeEvent, true, customerExpansion)
            )
                ?.Customer
                ?.Metadata,

            HandledStripeWebhook.UpcomingInvoice => await GetCustomerMetadataFromUpcomingInvoiceEvent(stripeEvent),

            HandledStripeWebhook.PaymentSucceeded
            or HandledStripeWebhook.PaymentFailed
            or HandledStripeWebhook.InvoiceCreated
            or HandledStripeWebhook.InvoiceFinalized => (await GetInvoice(stripeEvent, true, customerExpansion))
                ?.Customer
                ?.Metadata,

            HandledStripeWebhook.PaymentMethodAttached => (
                await GetPaymentMethod(stripeEvent, true, customerExpansion)
            )
                ?.Customer
                ?.Metadata,

            HandledStripeWebhook.CustomerUpdated => (await GetCustomer(stripeEvent, true))?.Metadata,
    ...
    private static string GetCustomerRegion(IDictionary<string, string> customerMetadata)
    {
        ...
        var miscasedRegionKey = customerMetadata.Keys.FirstOrDefault(key =>
        ...
        return !string.IsNullOrWhiteSpace(regionValue) ? regionValue : defaultRegion;
    }
}
