...
public class UpdateSecretsManagerSubscriptionCommand : IUpdateSecretsManagerSubscriptionCommand
{
    ...
    private async Task FinalizeSubscriptionAdjustmentAsync(SecretsManagerSubscriptionUpdate update)
    {
        ...
        {
            await _paymentService.AdjustSmSeatsAsync(
                update.Organization,
                update.Plan,
        ...
        {
            await _paymentService.AdjustServiceAccountsAsync(
                update.Organization,
                update.Plan,
    ...
    private async Task SendSeatLimitEmailAsync(Organization organization)
    {
        try
        {
            ...
            await _mailService.SendSecretsManagerMaxSeatLimitReachedEmailAsync(
                organization,
    ...
    private async Task SendServiceAccountLimitEmailAsync(Organization organization)
    {
        ...
        catch (Exception e)
        {
            _logger.LogError(
                e,
                $"Error encountered notifying organization owners of machine accounts limit reached."
            );
        }
    }
    ...
    private async Task ValidateSmSeatsUpdateAsync(SecretsManagerSubscriptionUpdate update)
    {
        ...
        if (
            ...
            || (
                plan.SecretsManager.MaxAdditionalSeats.HasValue
                && update.SmSeatsExcludingBase > plan.SecretsManager.MaxAdditionalSeats.Value
            )
        )
        {
            var planMaxSeats =
                plan.SecretsManager.BaseSeats + plan.SecretsManager.MaxAdditionalSeats.GetValueOrDefault();
            throw new BadRequestException(
                $"You have reached the maximum number of Secrets Manager seats ({planMaxSeats}) for this plan."
            );
        }
        ...
        {
            throw new BadRequestException(
                $"Plan has a minimum of {plan.SecretsManager.BaseSeats} Secrets Manager  seats."
            );
        }
        ...
        {
            var occupiedSeats = await _organizationUserRepository.GetOccupiedSmSeatCountByOrganizationIdAsync(
                organization.Id
            );
            if (occupiedSeats > update.SmSeats.Value)
            {
                throw new BadRequestException(
                    $"{occupiedSeats} users are currently occupying Secrets Manager seats. "
    ...
    private async Task ValidateSmServiceAccountsUpdateAsync(SecretsManagerSubscriptionUpdate update)
    {
        ...
        if (
            ...
            || (
                plan.SecretsManager.MaxAdditionalServiceAccount.HasValue
                && update.SmServiceAccountsExcludingBase > plan.SecretsManager.MaxAdditionalServiceAccount.Value
            )
        )
        {
            var planMaxServiceAccounts =
                plan.SecretsManager.BaseServiceAccount
                + plan.SecretsManager.MaxAdditionalServiceAccount.GetValueOrDefault();
            throw new BadRequestException(
                $"You have reached the maximum number of machine accounts ({planMaxServiceAccounts}) for this plan."
            );
        }

        // Check autoscale maximum service accounts
        if (
            update.MaxAutoscaleSmServiceAccounts.HasValue
        ...
        {
            throw new BadRequestException(
                $"Plan has a minimum of {plan.SecretsManager.BaseServiceAccount} machine accounts."
            );
        }
        ...
        if (
            ...
            || organization.SmServiceAccounts.Value > update.SmServiceAccounts.Value
        )
        {
    ...
    private void ValidateMaxAutoscaleSmSeatsUpdateAsync(SecretsManagerSubscriptionUpdate update)
    {
        ...
        {
            throw new BadRequestException(
                $"Cannot set max Secrets Manager seat autoscaling below current Secrets Manager seat count."
            );
        }

        if (
            plan.SecretsManager.MaxSeats.HasValue
            && update.MaxAutoscaleSmSeats.Value > plan.SecretsManager.MaxSeats
        )
        {
            throw new BadRequestException(
                string.Concat(
    ...
    private void ValidateMaxAutoscaleSmServiceAccountUpdate(SecretsManagerSubscriptionUpdate update)
    {
        ...
        if (
            update.SmServiceAccounts.HasValue
            && update.MaxAutoscaleSmServiceAccounts.Value < update.SmServiceAccounts.Value
        )
        {
            throw new BadRequestException(
        ...
        if (
            plan.SecretsManager.MaxServiceAccounts.HasValue
            && update.MaxAutoscaleSmServiceAccounts.Value > plan.SecretsManager.MaxServiceAccounts
        )
        {
            throw new BadRequestException(
                string.Concat(
