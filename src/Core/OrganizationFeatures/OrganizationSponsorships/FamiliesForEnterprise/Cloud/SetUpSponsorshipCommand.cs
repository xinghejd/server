...
public class SetUpSponsorshipCommand : ISetUpSponsorshipCommand
{
    ...
    )
    {
        ...
        var requiredSponsoredProductType = StaticStore
            .GetSponsoredPlan(sponsorship.PlanSponsorshipType.Value)
            ?.SponsoredProductTierType;
        if (
            requiredSponsoredProductType == null
            || sponsoredOrganization == null
            || StaticStore.GetPlan(sponsoredOrganization.PlanType).ProductTier
                != requiredSponsoredProductType.Value
        )
        {
