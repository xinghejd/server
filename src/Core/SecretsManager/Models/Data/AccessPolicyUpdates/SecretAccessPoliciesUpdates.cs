...
public class SecretAccessPoliciesUpdates
{
    public SecretAccessPoliciesUpdates(SecretAccessPolicies accessPolicies)
    {
        ...
        UserAccessPolicyUpdates = accessPolicies.UserAccessPolicies.Select(x => new UserSecretAccessPolicyUpdate
        {
            Operation = AccessPolicyOperation.Create,
            AccessPolicy = x,
        });

        GroupAccessPolicyUpdates = accessPolicies.GroupAccessPolicies.Select(x => new GroupSecretAccessPolicyUpdate
        {
            Operation = AccessPolicyOperation.Create,
            AccessPolicy = x,
        });

        ServiceAccountAccessPolicyUpdates = accessPolicies.ServiceAccountAccessPolicies.Select(
            x => new ServiceAccountSecretAccessPolicyUpdate
            {
                Operation = AccessPolicyOperation.Create,
                AccessPolicy = x,
            }
    ...
    public bool HasUpdates() =>
        UserAccessPolicyUpdates.Any() || GroupAccessPolicyUpdates.Any() || ServiceAccountAccessPolicyUpdates.Any();
}
