...
public class NotificationHubPool : INotificationHubPool
{
    ...
    private List<NotificationHubConnection> FilterInvalidHubs(
        IEnumerable<GlobalSettings.NotificationHubSettings> hubs
    )
    {
        ...
        foreach (var hub in hubs)
        {
            ...
            {
                _logger.LogWarning(
                    "Invalid notification hub settings: {HubName}",
                    hub.HubName ?? "hub name missing"
                );
                continue;
            }
    ...
    public NotificationHubClient ClientFor(Guid comb)
    {
        ...
        if (possibleConnections.Length == 0)
        {
            throw new InvalidOperationException(
                $"No valid notification hubs are available for the given comb ({comb}).\n"
                    + $"The comb's datetime is {CoreHelpers.DateFromComb(comb)}."
                    + $"Hub start and end times are configured as follows:\n"
                    + string.Join(
                        "\n",
                        _connections.Select(c =>
                            $"Hub {c.HubName} - Start: {c.RegistrationStartDate}, End: {c.RegistrationEndDate}"
                        )
                    )
            );
        }
        ...
        _logger.LogTrace(
            "Resolved notification hub for comb {Comb} out of {HubCount} hubs.\n{ConnectionInfo}",
            comb,
            possibleConnections.Length,
            resolvedConnection.LogString
        );
    ...
    {
        get { return new NotificationHubClientProxy(_clients); }
    }
}
