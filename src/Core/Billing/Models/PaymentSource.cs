...
public record PaymentSource(PaymentMethodType Type, string Description, bool NeedsVerification)
{
    ...
    public static PaymentSource From(Stripe.SetupIntent setupIntent)
    {
        ...
        return new PaymentSource(PaymentMethodType.BankAccount, description, true);
    }
    ...
    private static PaymentSource FromStripeBankAccountPaymentMethod(Stripe.PaymentMethodUsBankAccount bankAccount)
    {
        var description = $"{bankAccount.BankName}, *{bankAccount.Last4}";

        return new PaymentSource(PaymentMethodType.BankAccount, description, false);
    }
    ...
    private static PaymentSource FromStripeBankAccountLegacySource(Stripe.BankAccount bankAccount)
    {
        ...
        return new PaymentSource(PaymentMethodType.BankAccount, description, needsVerification);
    }
    ...
    private static string GetCardDescription(
        ...
        long expirationYear
    ) => $"{brand.ToUpperInvariant()}, *{last4}, {expirationMonth:00}/{expirationYear}";
}
