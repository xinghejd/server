...
) : ISubscriberService
{
    ...
    public async Task<string> CreateBraintreeCustomer(ISubscriber subscriber, string paymentMethodNonce)
    {
        ...
        var customerResult = await braintreeGateway.Customer.CreateAsync(
            new CustomerRequest
            {
    ...
    public async Task<Customer> GetCustomer(ISubscriber subscriber, CustomerGetOptions customerGetOptions = null)
    {
        ...
        {
            logger.LogError(
                "Cannot retrieve customer for subscriber ({SubscriberID}) with no {FieldName}",
                subscriber.Id,
                nameof(subscriber.GatewayCustomerId)
            );
        ...
        try
        {
            ...
            logger.LogError(
                "Could not find Stripe customer ({CustomerID}) for subscriber ({SubscriberID})",
        ...
        {
            logger.LogError(
                "An error occurred while trying to retrieve Stripe customer ({CustomerID}) for subscriber ({SubscriberID}): {Error}",
                subscriber.GatewayCustomerId,
                subscriber.Id,
                exception.Message
            );
    ...
    public async Task<PaymentMethod> GetPaymentMethod(ISubscriber subscriber)
    {
        ArgumentNullException.ThrowIfNull(subscriber);
        ...
        var subscriptionStatus = customer
            .Subscriptions.FirstOrDefault(subscription => subscription.Id == subscriber.GatewaySubscriptionId)
            ?.Status;

        var taxInformation = GetTaxInformation(customer);

        return new PaymentMethod(accountCredit, paymentMethod, subscriptionStatus, taxInformation);
    }

    public async Task<PaymentSource> GetPaymentSource(ISubscriber subscriber)
    {
        ArgumentNullException.ThrowIfNull(subscriber);

        var customer = await GetCustomerOrThrow(
            subscriber,
    ...
    )
    {
        ...
        {
            logger.LogError(
                "Cannot retrieve subscription for subscriber ({SubscriberID}) with no {FieldName}",
                subscriber.Id,
                nameof(subscriber.GatewaySubscriptionId)
            );
        ...
        try
        {
            ...
            logger.LogError(
                "Could not find Stripe subscription ({SubscriptionID}) for subscriber ({SubscriberID})",
                subscriber.GatewaySubscriptionId,
                subscriber.Id
            );
        ...
        {
            logger.LogError(
                "An error occurred while trying to retrieve Stripe subscription ({SubscriptionID}) for subscriber ({SubscriberID}): {Error}",
                subscriber.GatewaySubscriptionId,
                subscriber.Id,
                exception.Message
            );
    ...
    public async Task<TaxInformation> GetTaxInformation(ISubscriber subscriber)
    {
        ArgumentNullException.ThrowIfNull(subscriber);
    ...
    public async Task RemovePaymentSource(ISubscriber subscriber)
    {
        ArgumentNullException.ThrowIfNull(subscriber);
        ...
        var stripeCustomer = await GetCustomerOrThrow(
            subscriber,
            new CustomerGetOptions { Expand = ["invoice_settings.default_payment_method", "sources"] }
        );
        ...
        if (stripeCustomer.Metadata?.TryGetValue(BraintreeCustomerIdKey, out var braintreeCustomerId) ?? false)
        {
            ...
            {
                logger.LogError(
                    "Failed to retrieve Braintree customer ({ID}) when removing payment method",
                    braintreeCustomerId
                );
            ...
            if (braintreeCustomer.DefaultPaymentMethod != null)
            {
                ...
                {
                    logger.LogError(
                        "Failed to update payment method for Braintree customer ({ID}) | Message: {Message}",
                        braintreeCustomerId,
                        updateCustomerResult.Message
                    );
                ...
                var deletePaymentMethodResult = await braintreeGateway.PaymentMethod.DeleteAsync(
                    existingDefaultPaymentMethod.Token
                );
                ...
                if (!deletePaymentMethodResult.IsSuccess())
                {
                    ...
                    logger.LogError(
                        ...
                        braintreeCustomerId,
                        deletePaymentMethodResult.Message
                    );
            ...
            {
                logger.LogWarning(
                    "Tried to remove non-existent Braintree payment method for Customer ({ID})",
                    braintreeCustomerId
                );
            }
        }
        else
        {
            ...
            var paymentMethods = stripeAdapter.PaymentMethodListAutoPagingAsync(
                new PaymentMethodListOptions { Customer = stripeCustomer.Id }
            );
    ...
    public async Task UpdatePaymentSource(ISubscriber subscriber, TokenizedPaymentSource tokenizedPaymentSource)
    {
        ArgumentNullException.ThrowIfNull(subscriber);
        ...
        switch (type)
        {
            ...
            {
                var getSetupIntentsForUpdatedPaymentMethod = stripeAdapter.SetupIntentList(
                    new SetupIntentListOptions { PaymentMethod = token }
                );

                var getExistingSetupIntentsForCustomer = stripeAdapter.SetupIntentList(
                    new SetupIntentListOptions { Customer = subscriber.GatewayCustomerId }
                );
                ...
                {
                    logger.LogError(
                        "There were more than 1 setup intents for subscriber's ({SubscriberID}) updated payment method",
                        subscriber.Id
                    );
                ...
                var existingSetupIntentsForCustomer = (await getExistingSetupIntentsForCustomer).Where(si =>
                ...
                var postProcessing = existingSetupIntentsForCustomer
                    .Select(si =>
                        stripeAdapter.SetupIntentCancel(
                            si.Id,
            ...
            case PaymentMethodType.Card:
            {
                ...
                await stripeAdapter.PaymentMethodAttachAsync(
                    token,
                ...
                postProcessing.Add(
                    stripeAdapter.CustomerUpdateAsync(
                        subscriber.GatewayCustomerId,
            ...
            case PaymentMethodType.PayPal:
            {
                ...
                {
                    var hasBraintreeCustomerId = customer.Metadata.TryGetValue(
                        BraintreeCustomerIdKey,
                        out braintreeCustomerId
                    );
                    ...
                    if (hasBraintreeCustomerId)
                    {
                        ...
                        {
                            logger.LogError(
                                "Failed to retrieve Braintree customer ({BraintreeCustomerId}) when updating payment method for subscriber ({SubscriberID})",
                                braintreeCustomerId,
                                subscriber.Id
                            );
            ...
            {
                logger.LogError(
                    "Cannot update subscriber's ({SubscriberID}) payment method to type ({PaymentMethodType}) as it is not supported",
                    subscriber.Id,
                    type.ToString()
                );
    ...
    public async Task UpdateTaxInformation(ISubscriber subscriber, TaxInformation taxInformation)
    {
        ArgumentNullException.ThrowIfNull(subscriber);
        ...
        if (!subscriber.IsUser())
        {
            ...
            if (!string.IsNullOrWhiteSpace(taxInformation.TaxId) && !string.IsNullOrWhiteSpace(taxIdType))
            {
                await stripeAdapter.TaxIdCreateAsync(
                    customer.Id,
        ...
        bool SubscriberIsEligibleForAutomaticTax(ISubscriber localSubscriber, Customer localCustomer) =>
            ...
            && (
                localCustomer.Subscriptions?.Any(sub =>
                    sub.Id == localSubscriber.GatewaySubscriptionId && !sub.AutomaticTax.Enabled
                ) ?? false
            )
            && localCustomer.Tax?.AutomaticTax == StripeConstants.AutomaticTaxStatus.Supported;
    }

    public async Task VerifyBankAccount(ISubscriber subscriber, string descriptorCode)
    {
        ...
        {
            logger.LogError(
                "No setup intent ID exists to verify for subscriber with ID ({SubscriberID})",
                subscriber.Id
            );
            throw new BillingException();
        }
        ...
        try
        {
            await stripeAdapter.SetupIntentVerifyMicroDeposit(
                setupIntentId,
        ...
        catch (StripeException stripeException)
        {
            if (!string.IsNullOrEmpty(stripeException.StripeError?.Code))
            {
                var message = stripeException.StripeError.Code switch
                {
                    StripeConstants.ErrorCodes.PaymentMethodMicroDepositVerificationAttemptsExceeded =>
                        "You have exceeded the number of allowed verification attempts. Please contact support.",
                    StripeConstants.ErrorCodes.PaymentMethodMicroDepositVerificationDescriptorCodeMismatch =>
            ...
            logger.LogError(
                stripeException,
                "An unhandled Stripe exception was thrown while verifying subscriber's ({SubscriberID}) bank account",
                subscriber.Id
            );
            throw new BillingException();
        }
    ...
    private async Task AddBraintreeCustomerIdAsync(Customer customer, string braintreeCustomerId)
    {
    ...
    private async Task<PaymentSource> GetPaymentSourceAsync(Guid subscriberId, Customer customer)
    {
        ...
        var setupIntent = await stripeAdapter.SetupIntentGet(
            setupIntentId,
    ...
    private static TaxInformation GetTaxInformation(Customer customer)
    {
    ...
    private async Task RemoveBraintreeCustomerIdAsync(Customer customer)
    {
        var metadata = customer.Metadata ?? new Dictionary<string, string>();
    ...
    private async Task RemoveStripePaymentMethodsAsync(Customer customer)
    {
    ...
    )
    {
        ...
        var createPaymentMethodResult = await braintreeGateway.PaymentMethod.CreateAsync(
            new PaymentMethodRequest { CustomerId = customer.Id, PaymentMethodNonce = defaultPaymentMethodToken }
        );

        if (!createPaymentMethodResult.IsSuccess())
        {
            logger.LogError(
                "Failed to replace payment method for Braintree customer ({ID}) - Creation of new payment method failed | Error: {Error}",
                customer.Id,
                createPaymentMethodResult.Message
            );
        ...
        {
            logger.LogError(
                "Failed to replace payment method for Braintree customer ({ID}) - Customer update failed | Error: {Error}",
                customer.Id,
                updateCustomerResult.Message
            );
