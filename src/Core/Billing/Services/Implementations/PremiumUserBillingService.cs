...
) : IPremiumUserBillingService
{
    ...
    private async Task<Customer> CreateCustomerAsync(User user, CustomerSetup customerSetup)
    {
        ...
        {
            logger.LogError("Cannot create customer for user ({UserID}) without a valid payment source", user.Id);
        ...
        {
            logger.LogError("Cannot create customer for user ({UserID}) without valid tax information", user.Id);
    ...
    private async Task<Subscription> CreateSubscriptionAsync(Guid userId, Customer customer, int? storage)
    {
        ...
        var subscriptionCreateOptions = new SubscriptionCreateOptions
        {
            ...
            PaymentBehavior = usingPayPal ? StripeConstants.PaymentBehavior.DefaultIncomplete : null,
            OffSession = true,
        };
