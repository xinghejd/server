...
public class AuthRequestUpdateProcessor
{
    ...
    public EventType OrganizationEventType =>
        ProcessedAuthRequest?.Approved.Value ?? false
    ...
    public void Process()
    {
        ...
        var isExpired =
            DateTime.UtcNow > _unprocessedAuthRequest.CreationDate.Add(_configuration.AuthRequestExpiresAfter);
        var isSpent =
    ...
    public async Task SendApprovalEmail(Func<OrganizationAdminAuthRequest, string, Task> callback)
    {
        ...
        var deviceTypeAndIdentifierDisplayString = string.IsNullOrWhiteSpace(
            _unprocessedAuthRequest.RequestDeviceIdentifier
        )
