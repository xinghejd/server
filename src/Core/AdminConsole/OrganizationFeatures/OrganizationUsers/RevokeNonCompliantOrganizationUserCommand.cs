...
public class RevokeNonCompliantOrganizationUserCommand(
...
) : IRevokeNonCompliantOrganizationUserCommand
{
    ...
    public async Task<CommandResult> RevokeNonCompliantOrganizationUsersAsync(
    ...
    private static (
        ...
        EventType eventType,
        DateTime? time
    ) GetRevokedUserEventTuple(OrganizationUserUserDetails organizationUser, DateTimeOffset dateTimeOffset) =>
    ...
    ) => new(organizationUser, EventType.OrganizationUser_Revoked, systemUser, dateTimeOffset.UtcDateTime);
    ...
    private async Task<CommandResult> ValidateAsync(RevokeOrganizationUsersRequest request)
    {
        ...
        if (
            request.ActionPerformedBy is StandardUser user
        ...
        if (
            !await confirmedOwnersExceptQuery.HasConfirmedOwnersExceptAsync(
        ...
        return request.OrganizationUsers.Aggregate(
            new CommandResult(),
            (result, userToRevoke) =>
            {
    ...
    private static bool NonOwnersCannotRevokeOwners(
        OrganizationUserUserDetails organizationUser,
        IActingUser actingUser
    ) =>
        actingUser is StandardUser { IsOrganizationOwnerOrProvider: false }
