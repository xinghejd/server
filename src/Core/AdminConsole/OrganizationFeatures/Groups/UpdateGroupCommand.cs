...
public class UpdateGroupCommand : IUpdateGroupCommand
{
    ...
    )
    {
        ...
        {
            await _eventService.LogOrganizationUserEventsAsync(
                users.Select(u => (u, EventType.OrganizationUser_UpdatedGroups, (DateTime?)eventDate))
            );
        }
    }
    ...
    {
        var collections = await _collectionRepository.GetManyByManyIdsAsync(collectionAccess.Select(c => c.Id));
    ...
    private async Task ValidateMemberAccessAsync(Group originalGroup, ICollection<Guid> memberAccess)
    {
