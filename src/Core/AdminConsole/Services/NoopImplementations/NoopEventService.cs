...
public class NoopEventService : IEventService
{
    ...
    public Task LogOrganizationUserEventAsync<T>(T organizationUser, EventType type, DateTime? date = null)
    ...
    public Task LogOrganizationUserEventsAsync<T>(IEnumerable<(T, EventType, EventSystemUser, DateTime?)> events)
        where T : IOrganizationUser
    {
        return Task.FromResult(0);
    }
