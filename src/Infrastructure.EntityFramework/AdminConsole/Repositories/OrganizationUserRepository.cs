...
public class OrganizationUserRepository
    : Repository<Core.Entities.OrganizationUser, OrganizationUser, Guid>,
        IOrganizationUserRepository
{
    ...
    public async Task<Guid> CreateAsync(
        Core.Entities.OrganizationUser obj,
    ...
    public override async Task DeleteAsync(Core.Entities.OrganizationUser organizationUser) =>
        await DeleteAsync(organizationUser.Id);
    ...
    public async Task DeleteAsync(Guid organizationUserId)
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var orgUser = await dbContext.OrganizationUsers.Where(ou => ou.Id == organizationUserId).FirstAsync();
            ...
            {
                var ssoUsers = dbContext.SsoUsers.Where(su =>
                    su.UserId == userId && su.OrganizationId == organizationId
                );
                dbContext.SsoUsers.RemoveRange(ssoUsers);
            }

            var collectionUsers = dbContext.CollectionUsers.Where(cu =>
                cu.OrganizationUserId == organizationUserId
            );
            dbContext.CollectionUsers.RemoveRange(collectionUsers);

            var groupUsers = dbContext.GroupUsers.Where(gu => gu.OrganizationUserId == organizationUserId);
            ...
            var orgSponsorships = await dbContext
                .OrganizationSponsorships.Where(os => os.SponsoringOrganizationUserId == organizationUserId)
                .ToListAsync();
    ...
    public async Task DeleteManyAsync(IEnumerable<Guid> organizationUserIds)
    {
        ...
        await dbContext
            .CollectionUsers.Where(cu => targetOrganizationUserIds.Contains(cu.OrganizationUserId))
            .ExecuteDeleteAsync();

        await dbContext
            .GroupUsers.Where(gu => targetOrganizationUserIds.Contains(gu.OrganizationUserId))
            .ExecuteDeleteAsync();

        await dbContext
            .UserProjectAccessPolicy.Where(ap => targetOrganizationUserIds.Contains(ap.OrganizationUserId!.Value))
            .ExecuteDeleteAsync();
        await dbContext
            .UserServiceAccountAccessPolicy.Where(ap =>
                targetOrganizationUserIds.Contains(ap.OrganizationUserId!.Value)
            )
            .ExecuteDeleteAsync();
        await dbContext
            .UserSecretAccessPolicy.Where(ap => targetOrganizationUserIds.Contains(ap.OrganizationUserId!.Value))
            .ExecuteDeleteAsync();

        await dbContext
            .OrganizationUsers.Where(ou => targetOrganizationUserIds.Contains(ou.Id))
            .ExecuteDeleteAsync();
    ...
    public async Task<
        Tuple<Core.Entities.OrganizationUser, ICollection<CollectionAccessSelection>>
    ...
    public async Task<Core.Entities.OrganizationUser> GetByOrganizationEmailAsync(
        Guid organizationId,
        string email
    )
    {
    ...
    public async Task<int> GetCountByOnlyOwnerAsync(Guid userId)
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            return await dbContext
                .OrganizationUsers.Where(ou =>
                    ou.Type == OrganizationUserType.Owner && ou.Status == OrganizationUserStatusType.Confirmed
                )
    ...
    {
        var query = new OrganizationUserReadCountByOrganizationIdEmailQuery(
            organizationId,
            email,
            onlyRegisteredUsers
        );
    ...
    public async Task<OrganizationUserOrganizationDetails> GetDetailsByUserAsync(
        Guid userId,
        Guid organizationId,
    ...
    public async Task<ICollection<Core.Entities.OrganizationUser>> GetManyAsync(IEnumerable<Guid> Ids)
    {
        ...
        {
            var dbContext = GetDatabaseContext(scope);
            var query = from ou in dbContext.OrganizationUsers where Ids.Contains(ou.Id) select ou;
            var data = await query.ToArrayAsync();
            return data;
        }
    }

    public async Task<ICollection<Core.Entities.OrganizationUser>> GetManyByManyUsersAsync(
    ...
    )
    {
        ...
        {
            var dbContext = GetDatabaseContext(scope);
            var query = from ou in dbContext.OrganizationUsers where userIds.Contains(ou.Id) select ou;
            return Mapper.Map<List<Core.Entities.OrganizationUser>>(await query.ToListAsync());
        }
    }

    public async Task<ICollection<Core.Entities.OrganizationUser>> GetManyByOrganizationAsync(
        Guid organizationId,
    ...
    )
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var query =
                from ou in dbContext.OrganizationUsers
                where ou.OrganizationId == organizationId && (type == null || ou.Type == type)
                select ou;
            return Mapper.Map<List<Core.Entities.OrganizationUser>>(await query.ToListAsync());
        }
    }
    ...
    public async Task<ICollection<Core.Entities.OrganizationUser>> GetManyByUserAsync(Guid userId)
    {
        ...
        {
            var dbContext = GetDatabaseContext(scope);
            var query = from ou in dbContext.OrganizationUsers where ou.UserId == userId select ou;
            return Mapper.Map<List<Core.Entities.OrganizationUser>>(await query.ToListAsync());
        }
    }

    public async Task<ICollection<OrganizationUserUserDetails>> GetManyDetailsByOrganizationAsync(
        Guid organizationId,
        bool includeGroups,
        ...
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            if (includeGroups)
            {
                groups = (
                    ...
                    .GroupBy(g => g.OrganizationUserId)
                    .ToList();
            }
            ...
            foreach (var user in users)
            {
                ...
                {
                    user.Groups =
                        groups.FirstOrDefault(g => g.Key == user.Id)?.Select(g => g.GroupId).ToList()
                        ?? new List<Guid>();
                }
    ...
    public async Task<ICollection<OrganizationUserOrganizationDetails>> GetManyDetailsByUserAsync(
        Guid userId,
        OrganizationUserStatusType? status = null
    )
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var query =
                from ou in view.Run(dbContext)
                where ou.UserId == userId && (status == null || ou.Status == status)
                select ou;
    ...
    )
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var data = await query
                .Select(x => new OrganizationUserPublicKey() { Id = x.ou.Id, PublicKey = x.u.PublicKey })
                .ToListAsync();
            return data;
        }
    }
    ...
    public async Task ReplaceAsync(
        Core.Entities.OrganizationUser obj,
        IEnumerable<CollectionAccessSelection> requestedCollections
    )
    {
        ...
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var existingCollectionUsers = await dbContext
                .CollectionUsers.Where(cu => cu.OrganizationUserId == obj.Id)
                .ToListAsync();
            ...
            foreach (var requestedCollection in requestedCollections)
            {
                var existingCollectionUser = existingCollectionUsers.FirstOrDefault(cu =>
                    cu.CollectionId == requestedCollection.Id
                );
                if (existingCollectionUser == null)
                {
                    // This is a brand new entry
                    dbContext.CollectionUsers.Add(
                        new CollectionUser
                        {
            ...
            dbContext.CollectionUsers.RemoveRange(
                existingCollectionUsers.Where(cu => !requestedCollectionIds.Contains(cu.CollectionId))
            );
            await dbContext.SaveChangesAsync();
        }
    }
    ...
    public async Task ReplaceManyAsync(IEnumerable<Core.Entities.OrganizationUser> organizationUsers)
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            await dbContext.UserBumpManyAccountRevisionDatesAsync(
                organizationUsers.Where(ou => ou.UserId.HasValue).Select(ou => ou.UserId.Value).ToArray()
            );
        }
    }

    public async Task<ICollection<string>> SelectKnownEmailsAsync(
        Guid organizationId,
    ...
    )
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var knownEmails =
                from e in emails
                where
                    (ouEmails.Contains(e) || uEmails.Contains(e))
    ...
    public async Task<IEnumerable<OrganizationUserUserDetails>> GetManyByMinimumRoleAsync(
        Guid organizationId,
        OrganizationUserType minRole
    )
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            var dbContext = GetDatabaseContext(scope);
            var query = dbContext
                .OrganizationUsers.Include(e => e.User)
                .Where(e =>
                    e.OrganizationId.Equals(organizationId)
                    && e.Type <= minRole
                    && e.Status == OrganizationUserStatusType.Confirmed
                )
                .Select(e => new OrganizationUserUserDetails() { Id = e.Id, Email = e.Email ?? e.User.Email });
    ...
    public async Task RestoreAsync(Guid id, OrganizationUserStatusType status)
    {
        ...
        {
            var dbContext = GetDatabaseContext(scope);
            var orgUser = await dbContext.OrganizationUsers.FirstOrDefaultAsync(ou =>
                ou.Id == id && ou.Status == OrganizationUserStatusType.Revoked
            );
    ...
    public async Task<IEnumerable<OrganizationUserPolicyDetails>> GetByUserIdWithPolicyDetailsAsync(
        Guid userId,
    ...
    )
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var query =
                ...
                where p.Type == policyType && (ou.UserId == userId || ou.Email == email)
    ...
    public async Task<
        IEnumerable<OrganizationUserResetPasswordDetails>
    ...
    )
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var data = await query
                .Select(x => new OrganizationUserResetPasswordDetails(x.ou, x.u, x.o))
                .ToListAsync();
            return data;
        }
    }
    ...
    )
    {
        return async (_, _) =>
        {
            ...
            var userOrganizationUsers = await GetDbSet(dbContext).Where(c => c.UserId == userId).ToListAsync();

            // Filter to only organization users that are included
            var validOrganizationUsers = userOrganizationUsers.Where(organizationUser =>
            ...
            {
                var updateOrganizationUser = newOrganizationUsers.First(newOrganizationUser =>
                    newOrganizationUser.Id == organizationUser.Id
                );
    ...
    public async Task<ICollection<Core.Entities.OrganizationUser>> GetManyByOrganizationWithClaimedDomainsAsync(
        Guid organizationId
    )
    {
        using (var scope = ServiceScopeFactory.CreateScope())
    ...
    public async Task RevokeManyByIdAsync(IEnumerable<Guid> organizationUserIds)
    {
        ...
        await dbContext
            .OrganizationUsers.Where(x => organizationUserIds.Contains(x.Id))
