...
public class GroupRepository : Repository<AdminConsoleEntities.Group, Group, Guid>, IGroupRepository
{
    ...
    public async Task CreateAsync(
        AdminConsoleEntities.Group obj,
    ...
    public async Task DeleteUserAsync(Guid groupId, Guid organizationUserId)
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var query =
                ...
                where gu.GroupId == groupId && gu.OrganizationUserId == organizationUserId
                select gu;
    ...
    public async Task<
        Tuple<AdminConsoleEntities.Group, ICollection<CollectionAccessSelection>>
    > GetByIdWithCollectionsAsync(Guid id)
    {
        ...
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var collections = query
                ...
                .ToList();
            return new Tuple<AdminConsoleEntities.Group, ICollection<CollectionAccessSelection>>(grp, collections);
        }
    }
    ...
    public async Task<
        ICollection<Tuple<AdminConsoleEntities.Group, ICollection<CollectionAccessSelection>>>
    > GetManyWithCollectionsByOrganizationIdAsync(Guid organizationId)
    {
        ...
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            return groups
                .Select(group => new Tuple<AdminConsoleEntities.Group, ICollection<CollectionAccessSelection>>(
                    group,
                    collections
    ...
    public async Task<ICollection<AdminConsoleEntities.Group>> GetManyByManyIds(IEnumerable<Guid> groupIds)
    {
        ...
        {
            var dbContext = GetDatabaseContext(scope);
            var query = from g in dbContext.Groups where groupIds.Contains(g.Id) select g;
            var groups = await query.ToListAsync();
            return Mapper.Map<List<AdminConsoleEntities.Group>>(groups);
        }
    }

    public async Task<ICollection<AdminConsoleEntities.GroupUser>> GetManyGroupUsersByOrganizationIdAsync(
        Guid organizationId
    )
    {
        using (var scope = ServiceScopeFactory.CreateScope())
    ...
    public async Task<ICollection<Guid>> GetManyUserIdsByIdAsync(Guid id)
    {
        ...
        {
            var dbContext = GetDatabaseContext(scope);
            var query = from gu in dbContext.GroupUsers where gu.GroupId == id select gu;
    ...
    public async Task ReplaceAsync(
        AdminConsoleEntities.Group group,
    ...
    )
    {
        ...
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var availableCollections = await dbContext
                .Collections.Where(c => c.OrganizationId == group.OrganizationId)
                .Select(c => c.Id)
                .ToListAsync();

            var existingCollectionGroups = await dbContext
                .CollectionGroups.Where(cg => cg.GroupId == group.Id)
                .ToListAsync();

            foreach (var requestedCollection in requestedCollections)
            {
                var existingCollectionGroup = existingCollectionGroups.FirstOrDefault(cg =>
                    cg.CollectionId == requestedCollection.Id
                );
    ...
    public async Task UpdateUsersAsync(Guid groupId, IEnumerable<Guid> organizationUserIds)
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var insert =
                ...
                select new GroupUser { GroupId = groupId, OrganizationUserId = ou.Id };
            await dbContext.AddRangeAsync(insert);

            var delete =
                from gu in dbContext.GroupUsers
                where gu.GroupId == groupId && !organizationUserIds.Contains(gu.OrganizationUserId)
                select gu;
    ...
    public async Task DeleteManyAsync(IEnumerable<Guid> groupIds)
    {
        ...
        {
            var dbContext = GetDatabaseContext(scope);
            var entities = await dbContext.Groups.Where(g => groupIds.Contains(g.Id)).ToListAsync();
