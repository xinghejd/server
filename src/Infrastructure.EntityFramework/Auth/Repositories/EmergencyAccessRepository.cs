        ...
        IEmergencyAccessRepository
{
    ...
    public async Task<ICollection<EmergencyAccessDetails>> GetExpiredRecoveriesAsync()
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            var dbContext = GetDatabaseContext(scope);
            var view = new EmergencyAccessDetailsViewQuery();
            var query = view.Run(dbContext).Where(ea => ea.Status == EmergencyAccessStatusType.RecoveryInitiated);
    ...
    )
    {
        return async (SqlConnection connection, SqlTransaction transaction) =>
        {
            ...
            var userEmergencyAccess = await GetDbSet(dbContext)
                ...
                .ToListAsync();
            var validEmergencyAccess = userEmergencyAccess.Where(ea => newKeys.Any(eak => eak.Id == ea.Id));
