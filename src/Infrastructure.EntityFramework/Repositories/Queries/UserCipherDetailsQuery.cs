    ...
    public virtual IQueryable<CipherDetails> Run(DatabaseContext dbContext)
    {
        var query =
            ...
            join ou in dbContext.OrganizationUsers
                on new
                {
                    CipherUserId = c.UserId,
                    c.OrganizationId,
                    UserId = _userId,
                    Status = OrganizationUserStatusType.Confirmed,
                } equals new
                {
                    CipherUserId = (Guid?)null,
                    OrganizationId = (Guid?)ou.OrganizationId,
                    ou.UserId,
                    ou.Status,
                }
            ...
            join o in dbContext.Organizations
                ...
                {
                    c.OrganizationId,
                    OuOrganizationId = ou.OrganizationId,
                    Enabled = true,
                } equals new
                {
                    OrganizationId = (Guid?)o.Id,
                    OuOrganizationId = o.Id,
                    o.Enabled,
                }

            join cc in dbContext.CollectionCiphers on c.Id equals cc.CipherId into cc_g
        ...
        var query2 = from c in dbContext.Ciphers where c.UserId == _userId select c;

        var union = query
            .Union(query2)
            .Select(c => new CipherDetails
            {
                Id = c.Id,
                UserId = c.UserId,
                ...
                Favorite =
                    _userId.HasValue
                    && c.Favorites != null
                    && c.Favorites.ToLowerInvariant().Contains($"\"{_userId}\":true"),
