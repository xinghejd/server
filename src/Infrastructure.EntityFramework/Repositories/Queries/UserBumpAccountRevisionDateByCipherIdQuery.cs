    ...
    public IQueryable<User> Run(DatabaseContext dbContext)
    {
        var query =
            ...
            join collectionCipher in dbContext.CollectionCiphers
                on _cipherId equals collectionCipher.CipherId
                into cc_g
            from cc in cc_g.DefaultIfEmpty()

            join collectionUser in dbContext.CollectionUsers
                on new { OrganizationUserId = ou.Id, cc.CollectionId } equals new
                {
                    collectionUser.OrganizationUserId,
                    collectionUser.CollectionId,
                }
                into cu_g
            ...
            join groupUser in dbContext.GroupUsers
                on new { CollectionId = (Guid?)cu.CollectionId, OrganizationUserId = ou.Id } equals new
                {
                    ...
                    groupUser.OrganizationUserId,
                }
                into gu_g
            from gu in gu_g.DefaultIfEmpty()

            join grp in dbContext.Groups on gu.GroupId equals grp.Id into g_g
            from g in g_g.DefaultIfEmpty()

            join collectionGroup in dbContext.CollectionGroups
                on new { gu.GroupId, cc.CollectionId } equals new
                {
