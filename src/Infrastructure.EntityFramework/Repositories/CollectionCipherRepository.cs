...
public class CollectionCipherRepository : BaseEntityFrameworkRepository, ICollectionCipherRepository
{
    ...
    public async Task<CollectionCipher> CreateAsync(CollectionCipher obj)
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var organizationId = (
                await dbContext.Ciphers.FirstOrDefaultAsync(c => c.Id.Equals(obj.CipherId))
            )?.OrganizationId;
    ...
    public async Task UpdateCollectionsAsync(Guid cipherId, Guid userId, IEnumerable<Guid> collectionIds)
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            foreach (var requestedCollectionId in collectionIds)
            {
                ...
                var existingCollectionCipher = collectionCiphers.FirstOrDefault(t =>
                    t.CollectionId == requestedCollectionId && t.CipherId == cipherId
                );
                ...
                if (existingCollectionCipher == null && availableCollections.Contains(requestedCollectionId))
                {
                    ...
                    dbContext.CollectionCiphers.Add(
                        new Models.CollectionCipher { CollectionId = requestedCollectionId, CipherId = cipherId }
                    );
                }
            ...
            dbContext.CollectionCiphers.RemoveRange(
                collectionCiphers.Where(cc => !collectionIds.Contains(cc.CollectionId) && cc.CipherId == cipherId)
            );
    ...
    )
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var currentCollectionCiphers = await (
                from cc in dbContext.CollectionCiphers
                where cc.CipherId == cipherId
                select cc
            ).ToListAsync();
            ...
            foreach (var requestedCollectionId in collectionIds)
            {
                var requestedCollectionCipher = currentCollectionCiphers.FirstOrDefault(cc =>
                    cc.CollectionId == requestedCollectionId
                );

                if (requestedCollectionCipher == null)
                {
                    dbContext.CollectionCiphers.Add(
                        new Models.CollectionCipher { CipherId = cipherId, CollectionId = requestedCollectionId }
                    );
                }
            }
    ...
    )
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var availableCollectionsQuery = new CollectionsReadByOrganizationIdUserIdQuery(organizationId, userId);
            var availableCollections = availableCollectionsQuery.Run(dbContext);
        ...
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var insertData =
                ...
                where
                    ...
                    && !currentCollectionCiphers.Any(cc =>
                        cc.CipherId == cipherId && cc.CollectionId == collectionId
                    )
                select new Models.CollectionCipher { CollectionId = collectionId, CipherId = cipherId };
    ...
    )
    {
        ...
        {
            var dbContext = GetDatabaseContext(scope);
            var currentCollectionCiphersToBeRemoved = await (
                from cc in dbContext.CollectionCiphers
