...
public class TransactionRepository
    : Repository<Core.Entities.Transaction, Transaction, Guid>,
        ITransactionRepository
{
    ...
    public async Task<Core.Entities.Transaction?> GetByGatewayIdAsync(GatewayType gatewayType, string gatewayId)
    {
        ...
        var results = await EntityFrameworkQueryableExtensions.FirstOrDefaultAsync(
            dbContext.Transactions,
            t => (t.GatewayId == gatewayId && t.Gateway == gatewayType)
        );
    ...
    )
    {
        ...
        var dbContext = GetDatabaseContext(scope);
        var query = dbContext.Transactions.Where(t => t.OrganizationId == organizationId && !t.UserId.HasValue);
    ...
    )
    {
        ...
        var query = databaseContext.Transactions.Where(transaction => transaction.ProviderId == providerId);
