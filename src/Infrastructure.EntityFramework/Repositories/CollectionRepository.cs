...
public class CollectionRepository : Repository<Core.Entities.Collection, Collection, Guid>, ICollectionRepository
{
    ...
    public override async Task<Core.Entities.Collection> CreateAsync(Core.Entities.Collection collection)
    {
        ...
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            await dbContext.UserBumpAccountRevisionDateByCollectionIdAsync(
                collection.Id,
    ...
    )
    {
        ...
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            if (users != null)
            {
                var availableUsers = await (
                    from g in dbContext.OrganizationUsers
                    where g.OrganizationId == obj.OrganizationId
                    select g.Id
                ).ToListAsync();
    ...
    public async Task DeleteUserAsync(Guid collectionId, Guid organizationUserId)
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var query =
                ...
                where cu.CollectionId == collectionId && cu.OrganizationUserId == organizationUserId
                select cu;
    ...
    public async Task<Tuple<Core.Entities.Collection?, CollectionAccessDetails>> GetByIdWithAccessAsync(Guid id)
    {
        ...
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var userQuery =
                from cg in dbContext.CollectionUsers
                where cg.CollectionId.Equals(id)
                select new CollectionAccessSelection
                {
    ...
    public async Task<
        ICollection<Tuple<Core.Entities.Collection, CollectionAccessDetails>>
    > GetManyByOrganizationIdWithAccessAsync(Guid organizationId)
    {
        ...
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            return collections
                .Select(collection => new Tuple<Core.Entities.Collection, CollectionAccessDetails>(
                    collection,
    ...
    public async Task<ICollection<Core.Entities.Collection>> GetManyByManyIdsAsync(IEnumerable<Guid> collectionIds)
    {
        ...
        {
            var dbContext = GetDatabaseContext(scope);
            var query = from c in dbContext.Collections where collectionIds.Contains(c.Id) select c;
    ...
    public async Task<ICollection<Core.Entities.Collection>> GetManyByOrganizationIdAsync(Guid organizationId)
    {
        ...
        {
            var dbContext = GetDatabaseContext(scope);
            var query = from c in dbContext.Collections where c.OrganizationId == organizationId select c;
            ...
            if (dbContext.Database.IsSqlite())
            {
                return (await baseCollectionQuery.ToListAsync())
                    ...
                    .Select(collectionGroup => new CollectionDetails
                    {
                        ...
                        HidePasswords = Convert.ToBoolean(
                            collectionGroup.Min(c => Convert.ToInt32(c.HidePasswords))
                        ),
    ...
    public async Task<CollectionAdminDetails?> GetByIdWithPermissionsAsync(
        Guid collectionId,
        Guid? userId,
        bool includeAccessRelationships
    )
    ...
    public async Task<ICollection<CollectionAccessSelection>> GetManyUsersByIdAsync(Guid id)
    {
        ...
        {
            var dbContext = GetDatabaseContext(scope);
            var query = from cu in dbContext.CollectionUsers where cu.CollectionId == id select cu;
    ...
    public async Task ReplaceAsync(
        Core.Entities.Collection collection,
        IEnumerable<CollectionAccessSelection> groups,
        IEnumerable<CollectionAccessSelection> users
    )
    {
    ...
    public async Task UpdateUsersAsync(Guid id, IEnumerable<CollectionAccessSelection> requestedUsers)
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            dbContext.CollectionUsers.RemoveRange(
                existingCollectionUsers.Where(cu => !requestedUserIds.Contains(cu.OrganizationUserId))
            );
    ...
    public async Task DeleteManyAsync(IEnumerable<Guid> collectionIds)
    {
        ...
        {
            var dbContext = GetDatabaseContext(scope);
            var collectionGroupEntities = await dbContext
                .CollectionGroups.Where(cg => collectionIds.Contains(cg.CollectionId))
                .ToListAsync();
            var collectionEntities = await dbContext
    ...
    public async Task CreateOrUpdateAccessForManyAsync(
        Guid organizationId,
    ...
    )
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            {
                var existingCollectionUsers = await dbContext
                    .CollectionUsers.Where(cu => collectionIdsList.Contains(cu.CollectionId))
                ...
                foreach (var collectionId in collectionIdsList)
                {
                    ...
                    {
                        if (
                            !existingCollectionUsers.TryGetValue(
            ...
            if (groups != null)
            {
                var existingCollectionGroups = await dbContext
                    .CollectionGroups.Where(cu => collectionIdsList.Contains(cu.CollectionId))
                ...
                foreach (var collectionId in collectionIdsList)
                {
                    ...
                    {
                        if (
                            !existingCollectionGroups.TryGetValue(
                        ...
                        {
                            // This is a brand new entry
                            dbContext.CollectionGroups.Add(
                                new CollectionGroup()
                                {
                                    CollectionId = collectionId,
    ...
    private static async Task ReplaceCollectionGroupsAsync(
    ...
    {
        var existingCollectionGroups = await dbContext
            .CollectionGroups.Where(cg => cg.CollectionId == collection.Id)
    ...
    private static async Task ReplaceCollectionUsersAsync(
        DatabaseContext dbContext,
        Core.Entities.Collection collection,
        IEnumerable<CollectionAccessSelection> users
    )
    {
        var existingCollectionUsers = await dbContext
            .CollectionUsers.Where(cu => cu.CollectionId == collection.Id)
        ...
        var toDelete = existingCollectionUsers.Values.Where(cu =>
            !requestedUserIds.Contains(cu.OrganizationUserId)
        );
