...
public static class DatabaseContextExtensions
{
    public static async Task UserBumpAccountRevisionDateAsync(this DatabaseContext context, Guid userId)
    {
        ...
        Debug.Assert(
            user is not null,
            "The user id is expected to be validated as a true-in database user before making this call."
        );
        user.AccountRevisionDate = DateTime.UtcNow;
    }

    public static async Task UserBumpManyAccountRevisionDatesAsync(
    ...
    public static async Task UserBumpAccountRevisionDateByOrganizationIdAsync(
        this DatabaseContext context,
        Guid organizationId
    )
    {
    ...
    public static async Task UserBumpAccountRevisionDateByCipherIdAsync(
        this DatabaseContext context,
        Guid cipherId,
        Guid organizationId
    )
    ...
    public static async Task UserBumpAccountRevisionDateByCollectionIdAsync(
        this DatabaseContext context,
        Guid collectionId,
        Guid organizationId
    )
    {
        var query =
            ...
            join cu in context.CollectionUsers
                on new { OrganizationUserId = ou.Id, CollectionId = collectionId } equals new
                {
            ...
            join g in context.Groups on gu.GroupId equals g.Id into g_g
            from g in g_g.DefaultIfEmpty()
            join cg in context.CollectionGroups
                on new { gu.GroupId, CollectionId = collectionId } equals new { cg.GroupId, cg.CollectionId }
                into cg_g
    ...
    public static async Task UserBumpAccountRevisionDateByCollectionIdsAsync(
        this DatabaseContext context,
    ...
    )
    {
        var query =
            ...
            join ou in context.OrganizationUsers on u.Id equals ou.UserId
            join cu in context.CollectionUsers
                on new { OrganizationUserId = ou.Id, CollectionId = c.Id } equals new
                {
                    cu.OrganizationUserId,
                    cu.CollectionId,
                }
            ...
            from g in g_g.DefaultIfEmpty()
            join cg in context.CollectionGroups
                on new { gu.GroupId, CollectionId = c.Id } equals new { cg.GroupId, cg.CollectionId }
                into cg_g
            from cg in cg_g.DefaultIfEmpty()
            where
                ou.OrganizationId == organizationId
                && collectionIds.Contains(c.Id)
                && ou.Status == OrganizationUserStatusType.Confirmed
                && (
                    (cu == null ? (Guid?)null : cu.CollectionId) != null
                    || (cg == null ? (Guid?)null : cg.CollectionId) != null
    ...
    public static async Task UserBumpAccountRevisionDateByOrganizationUserIdAsync(
        this DatabaseContext context,
    ...
    public static async Task UserBumpAccountRevisionDateByOrganizationUserIdsAsync(
        this DatabaseContext context,
    ...
    public static async Task UserBumpAccountRevisionDateByEmergencyAccessGranteeIdAsync(
        this DatabaseContext context,
    ...
    public static async Task UserBumpAccountRevisionDateByProviderIdAsync(
        this DatabaseContext context,
        Guid providerId
    )
    {
    ...
    public static async Task UserBumpAccountRevisionDateByProviderUserIdAsync(
        this DatabaseContext context,
        Guid providerUserId
    )
    {
