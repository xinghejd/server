...
public class FolderRepository : Repository<Core.Vault.Entities.Folder, Folder, Guid>, IFolderRepository
{
    ...
    )
    {
        return async (SqlConnection _, SqlTransaction _) =>
        {
            ...
            using var scope = ServiceScopeFactory.CreateScope();
            var dbContext = GetDatabaseContext(scope);
            var userFolders = await GetDbSet(dbContext).Where(f => f.UserId == userId).ToListAsync();
            var validFolders = userFolders.Where(folder => newFolders.Any(newFolder => newFolder.Id == folder.Id));
