...
public class AccountController : Controller
{
    ...
    public async Task<IActionResult> ExternalCallback()
    {
        ...
        var (user, provider, providerUserId, claims, ssoConfigData) = await FindUserFromExternalProviderAsync(
            result
        );
        if (user == null)
        {
    ...
    )
    {
        ...
        {
            var organizationDomain = await _organizationDomainRepository.GetDomainByOrgIdAndDomainNameAsync(
                orgId,
                emailDomain
            );
    ...
    private string GetEmailAddress(IEnumerable<Claim> claims, IEnumerable<string> additionalClaimTypes)
    {
        ...
        var username = filteredClaims.GetFirstMatch(JwtClaimTypes.PreferredUserName, SamlClaimTypes.UserId, "uid");
