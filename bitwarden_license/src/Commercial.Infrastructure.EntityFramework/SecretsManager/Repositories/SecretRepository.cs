...
public class SecretRepository : Repository<Core.SecretsManager.Entities.Secret, Secret, Guid>, ISecretRepository
{
    ...
    public async Task<IEnumerable<Core.SecretsManager.Entities.Secret>> GetManyByOrganizationIdInTrashByIdsAsync(
        Guid organizationId,
    ...
    )
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var secrets = await dbContext
                .Secret.Where(s =>
                    ids.Contains(s.Id) && s.OrganizationId == organizationId && s.DeletedDate != null
                )
                .Include("Projects")
    ...
    public async Task<IEnumerable<SecretPermissionDetails>> GetManyDetailsByOrganizationIdInTrashAsync(
        Guid organizationId
    )
    {
        using (var scope = ServiceScopeFactory.CreateScope())
    ...
    public async Task SoftDeleteManyByIdAsync(IEnumerable<Guid> ids)
    {
        ...
        await dbContext
            ...
            .ExecuteUpdateAsync(setters =>
                setters.SetProperty(s => s.RevisionDate, utcNow).SetProperty(s => s.DeletedDate, utcNow)
            );
    ...
    public async Task RestoreManyByIdAsync(IEnumerable<Guid> ids)
    {
        ...
        await dbContext
            .Secret.Where(c => secretIds.Contains(c.Id))
            .ExecuteUpdateAsync(setters =>
                setters.SetProperty(s => s.RevisionDate, utcNow).SetProperty(s => s.DeletedDate, (DateTime?)null)
            );
    ...
    public async Task EmptyTrash(DateTime currentDate, uint deleteAfterThisNumberOfDays)
    {
        ...
        await dbContext
            .Secret.Where(s =>
                s.DeletedDate != null && s.DeletedDate < currentDate.AddDays(-deleteAfterThisNumberOfDays)
            )
            .ExecuteDeleteAsync();
    ...
    private static Expression<Func<Secret, bool>> ServiceAccountHasReadAccessToSecret(Guid serviceAccountId) =>
        s =>
    ...
    )
    {
        ...
        var groupAccessPoliciesIdsToDelete = entity
            .GroupAccessPolicies.Where(gap =>
                accessPoliciesUpdates.GroupAccessPolicyUpdates.Any(apu =>
                    apu.Operation == AccessPolicyOperation.Delete && apu.AccessPolicy.GroupId == gap.GroupId
                )
            )
            .Select(gap => gap.Id)
            .ToList();
        ...
        await dbContext.AccessPolicies.Where(ap => accessPoliciesIdsToDelete.Contains(ap.Id)).ExecuteDeleteAsync();
    }
    ...
    )
    {
        ...
        await UpdateServiceAccountRevisionsAsync(
            ...
            accessPoliciesUpdates
                .ServiceAccountAccessPolicyUpdates.Select(sap => sap.AccessPolicy.ServiceAccountId!.Value)
                .ToList()
        );
    }
