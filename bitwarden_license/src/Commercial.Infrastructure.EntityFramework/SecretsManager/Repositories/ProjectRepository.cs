...
public class ProjectRepository
    : Repository<Core.SecretsManager.Entities.Project, Project, Guid>,
        IProjectRepository
{
    ...
    public override async Task<Core.SecretsManager.Entities.Project?> GetByIdAsync(Guid id)
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            var project = await dbContext
                .Project.Where(c => c.Id == id && c.DeletedDate == null)
                .FirstOrDefaultAsync();
    ...
    )
    {
        ...
        var query = dbContext
            .Project.Where(p => p.OrganizationId == organizationId && p.DeletedDate == null)
    ...
    public async Task<int> GetProjectCountByOrganizationIdAsync(Guid organizationId)
    {
        ...
        {
            var dbContext = GetDatabaseContext(scope);
            return await dbContext.Project.CountAsync(ou => ou.OrganizationId == organizationId);
        }
    }
    ...
    public async Task DeleteManyByIdAsync(IEnumerable<Guid> ids)
    {
        ...
        if (secretIds.Count > 0)
        {
            await dbContext
                ...
                .ExecuteUpdateAsync(setters => setters.SetProperty(s => s.RevisionDate, utcNow));
        }
    ...
    public async Task<IEnumerable<Core.SecretsManager.Entities.Project>> GetManyWithSecretsByIds(
    ...
    public async Task<IEnumerable<Core.SecretsManager.Entities.Project>> ImportAsync(
        IEnumerable<Core.SecretsManager.Entities.Project> projects
    )
    {
        using var scope = ServiceScopeFactory.CreateScope();
    ...
    public async Task<(bool Read, bool Write)> AccessToProjectAsync(
        Guid id,
        Guid userId,
        AccessClientType accessType
    )
    {
        ...
        var projectQuery = dbContext.Project.Where(s => s.Id == id);
    ...
    public async Task<bool> ProjectsAreInOrganization(List<Guid> projectIds, Guid organizationId)
    {
        ...
        var results = await dbContext
            .Project.Where(p => p.OrganizationId == organizationId && projectIds.Contains(p.Id))
            .ToListAsync();
    ...
    public async Task<int> GetProjectCountByOrganizationIdAsync(
        Guid organizationId,
        Guid userId,
        AccessClientType accessType
    )
    ...
    public async Task<ProjectCounts> GetProjectCountsByIdAsync(
        Guid projectId,
        Guid userId,
        AccessClientType accessType
    )
    {
        ...
        var projectCounts =
            await projectCountsQuery.FirstOrDefaultAsync()
            ?? new ProjectCounts
            {
                Secrets = 0,
                People = 0,
    ...
    private static IQueryable<ProjectAccess> BuildProjectAccessQuery(
    ...
    private IQueryable<ProjectPermissionDetails> ProjectToPermissionDetails(
    ...
    )
    {
        var projects = accessType switch
        {
            ...
            AccessClientType.User => query
                ...
                .Select(ProjectToPermissionsUser(userId, true)),
            AccessClientType.ServiceAccount => query
                .Where(ServiceAccountHasReadAccessToProject(userId))
                .Select(ProjectToPermissionsServiceAccount(userId, true)),
            _ => throw new ArgumentOutOfRangeException(nameof(accessType), accessType, null),
        };
        return projects;
    }
    ...
    private Expression<Func<Project, ProjectPermissionDetails>> ProjectToPermissionsServiceAccount(
        Guid userId,
        bool read
    ) =>
        p => new ProjectPermissionDetails
    ...
    private static Expression<Func<Project, bool>> UserHasReadAccessToProject(Guid userId) =>
        p =>
            p.UserAccessPolicies.Any(ap => ap.OrganizationUser.User.Id == userId && ap.Read)
            || p.GroupAccessPolicies.Any(ap =>
                ap.Group.GroupUsers.Any(gu => gu.OrganizationUser.User.Id == userId && ap.Read)
    ...
    private static Expression<Func<Project, bool>> UserHasWriteAccessToProject(Guid userId) =>
        p =>
            p.UserAccessPolicies.Any(ap => ap.OrganizationUser.User.Id == userId && ap.Write)
            || p.GroupAccessPolicies.Any(ap =>
                ap.Group.GroupUsers.Any(gu => gu.OrganizationUser.User.Id == userId && ap.Write)
    ...
    private static Expression<Func<Project, bool>> ServiceAccountHasReadAccessToProject(Guid serviceAccountId) =>
        p => p.ServiceAccountAccessPolicies.Any(ap => ap.ServiceAccount.Id == serviceAccountId && ap.Read);
}
