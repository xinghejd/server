...
public class BulkSecretAuthorizationHandler
    : AuthorizationHandler<BulkSecretOperationRequirement, IReadOnlyList<Secret>>
{
    ...
    )
    {
        ...
        if (
            resources.Any(secret => secret.OrganizationId != organizationId)
    ...
    private async Task CanReadAllAsync(
        AuthorizationHandlerContext context,
        BulkSecretOperationRequirement requirement,
        IReadOnlyList<Secret> resources,
        Guid organizationId
    )
    {
        ...
        var secretsAccess = await _secretRepository.AccessToSecretsAsync(
            resources.Select(s => s.Id),
            userId,
            accessClient
        );

        if (secretsAccess.Count == resources.Count && secretsAccess.All(a => a.Value.Read))
        {
