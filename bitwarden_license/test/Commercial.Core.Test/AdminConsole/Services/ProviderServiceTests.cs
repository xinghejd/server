...
public class ProviderServiceTests
{
    ...
    [Theory, BitAutoData]
    public async Task CompleteSetupAsync_TokenIsInvalid_Throws(
        User user,
        Provider provider,
    ...
    [Theory, BitAutoData]
    public async Task CompleteSetupAsync_Success(
        User user,
        Provider provider,
        string key,
        TaxInfo taxInfo,
    ...
    )
    {
        ...
        await sutProvider
            ...
            .Received()
            .ReplaceAsync(
                Arg.Is<ProviderUser>(pu => pu.UserId == user.Id && pu.ProviderId == provider.Id && pu.Key == key)
            );
    }

    [Theory, BitAutoData]
    public async Task UpdateAsync_ProviderIdIsInvalid_Throws(
        Provider provider,
        SutProvider<ProviderService> sutProvider
    )
    {
        ...
        var exception = await Assert.ThrowsAsync<ArgumentException>(() => sutProvider.Sut.UpdateAsync(provider));
    ...
    [Theory, BitAutoData]
    public async Task InviteUserAsync_ProviderIdIsInvalid_Throws(
        ProviderUserInvite<string> invite,
        SutProvider<ProviderService> sutProvider
    )
    {
    ...
    [Theory, BitAutoData]
    public async Task InviteUserAsync_InvalidPermissions_Throws(
        ProviderUserInvite<string> invite,
        SutProvider<ProviderService> sutProvider
    )
    {
    ...
    [Theory, BitAutoData]
    public async Task InviteUserAsync_EmailsInvalid_Throws(
        Provider provider,
        ProviderUserInvite<string> providerUserInvite,
        SutProvider<ProviderService> sutProvider
    )
    {
        ...
        sutProvider
            ...
            .ProviderManageUsers(providerUserInvite.ProviderId)
            .Returns(true);
    ...
    [Theory, BitAutoData]
    public async Task InviteUserAsync_AlreadyInvited(
        Provider provider,
        ProviderUserInvite<string> providerUserInvite,
        SutProvider<ProviderService> sutProvider
    )
    ...
    [Theory, BitAutoData]
    public async Task InviteUserAsync_Success(
        Provider provider,
        ProviderUserInvite<string> providerUserInvite,
        SutProvider<ProviderService> sutProvider
    )
    {
        ...
        Assert.True(
            result.TrueForAll(pu => pu.Status == ProviderUserStatusType.Invited),
            "Status must be invited"
        );
        Assert.True(
            result.TrueForAll(pu => pu.ProviderId == providerUserInvite.ProviderId),
            "Provider Id must be correct"
        );
    }

    [Theory, BitAutoData]
    public async Task ResendInviteUserAsync_InvalidPermissions_Throws(
        ProviderUserInvite<Guid> invite,
        SutProvider<ProviderService> sutProvider
    )
    {
        sutProvider.GetDependency<ICurrentContext>().ProviderManageUsers(invite.ProviderId).Returns(false);
    ...
    [Theory, BitAutoData]
    public async Task ResendInvitesAsync_Errors(
        Provider provider,
    ...
    [Theory, BitAutoData]
    public async Task ResendInvitesAsync_Success(
        Provider provider,
        IEnumerable<ProviderUser> providerUsers,
        SutProvider<ProviderService> sutProvider
    )
    {
    ...
    [Theory, BitAutoData]
    public async Task SendProviderSetupInviteEmailAsync_Success(
        Provider provider,
        string email,
        SutProvider<ProviderService> sutProvider
    )
    {
        ...
        await sutProvider
            ...
            .Received(1)
            .SendProviderSetupInviteEmailAsync(provider, Arg.Any<string>(), email);
    }
    ...
    public async Task AcceptUserAsync_AlreadyAccepted_Throws(
        [ProviderUser(ProviderUserStatusType.Accepted)] ProviderUser providerUser,
        User user,
        SutProvider<ProviderService> sutProvider
    )
    {
    ...
    )
    {
        ...
        var token = protector.Protect(
            $"ProviderUserInvite {providerUser.Id} {user.Email} {CoreHelpers.ToEpocMilliseconds(DateTime.UtcNow)}"
        );
    ...
    [Theory, BitAutoData]
    public async Task SaveUserAsync_UserIdIsInvalid_Throws(
    ...
    public async Task SaveUserAsync_Success(
        [ProviderUser(type: ProviderUserType.ProviderAdmin)] ProviderUser providerUser,
        User savingUser,
        SutProvider<ProviderService> sutProvider
    )
    {
    ...
    [Theory, BitAutoData]
    public async Task DeleteUsersAsync_NoRemainingOwner_Throws(
        Provider provider,
        User deletingUser,
    ...
    [Theory, BitAutoData]
    public async Task DeleteUsersAsync_Success(
        Provider provider,
        User deletingUser,
        ICollection<ProviderUser> providerUsers,
        [ProviderUser(ProviderUserStatusType.Confirmed, ProviderUserType.ProviderAdmin)]
            ProviderUser remainingOwner,
        SutProvider<ProviderService> sutProvider
    )
    {
        var userIds = providerUsers.Select(pu => pu.Id);
        ...
        providerUserRepository
            .GetManyByProviderAsync(default, default)
            .ReturnsForAnyArgs(new[] { remainingOwner });
    ...
    [Theory, BitAutoData]
    public async Task AddOrganization_OrganizationAlreadyBelongsToAProvider_Throws(
        Provider provider,
    ...
    [Theory, BitAutoData]
    public async Task AddOrganization_OrganizationHasSecretsManager_Throws(
        Provider provider,
        Organization organization,
        string key,
        SutProvider<ProviderService> sutProvider
    ...
    )
    {
        ...
        await providerOrganizationRepository
            .Received(1)
            .CreateAsync(
                Arg.Is<ProviderOrganization>(providerOrganization =>
        ...
        await sutProvider
            .GetDependency<IEventService>()
            .Received()
            .LogProviderOrganizationEventAsync(
                Arg.Is<ProviderOrganization>(providerOrganization =>
                    providerOrganization.ProviderId == provider.Id
                    && providerOrganization.OrganizationId == organization.Id
                    && providerOrganization.Key == key
    ...
    [Theory, BitAutoData]
    public async Task AddOrganization_CreateAfterNov62023_PlanTypeDoesNotUpdated(
        Provider provider,
        Organization organization,
        string key,
        SutProvider<ProviderService> sutProvider
    ...
    [Theory, BitAutoData]
    public async Task AddOrganization_CreateBeforeNov62023_PlanTypeUpdated(
        Provider provider,
        Organization organization,
        string key,
        SutProvider<ProviderService> sutProvider
    )
    {
        ...
        await sutProvider
            ...
            .SubscriptionUpdateAsync(
                ...
                SubscriptionUpdateRequest(expectedPlanId, subscriptionItem)
            );
    ...
    [Theory, BitAutoData]
    public async Task AddOrganizationsToReseller_WithResellerProvider_Success(
        Provider provider,
    ...
    )
    {
        ...
        await providerOrganizationRepository
            .Received(1)
            .CreateManyAsync(
                Arg.Is<IEnumerable<ProviderOrganization>>(i =>
        ...
        await sutProvider
            .GetDependency<IEventService>()
            .Received(1)
            .LogProviderOrganizationEventsAsync(
                Arg.Is<IEnumerable<(ProviderOrganization, EventType, DateTime?)>>(events =>
                    events.All(e =>
                        e.Item1.ProviderId == provider.Id
                        && organizationIds.Contains(e.Item1.OrganizationId)
    ...
    [Theory, BitAutoData]
    public async Task AddOrganizationsToReseller_WithMspProvider_Throws(
        Provider provider,
        ICollection<Organization> organizations,
        SutProvider<ProviderService> sutProvider
    )
    {
        ...
        await sutProvider
            ...
            .LogProviderOrganizationEventsAsync(default);
    }
    ...
    )
    {
        ...
        sutProvider
            ...
            .SignupClientAsync(organizationSignup)
            .Returns((organization, null as OrganizationUser, new Collection()));

        var providerOrganization = await sutProvider.Sut.CreateOrganizationAsync(
            provider.Id,
            organizationSignup,
            clientOwnerEmail,
            user
        ...
        await sutProvider
            ...
            .InviteUsersAsync(
                ...
                user.Id,
                systemUser: null,
                Arg.Is<IEnumerable<(OrganizationUserInvite, string)>>(t =>
                    t.Count() == 1
    ...
    )
    {
        ...
        await providerOrganizationRepository
            .Received(1)
            .CreateAsync(
                Arg.Is<ProviderOrganization>(po =>
                    po.ProviderId == provider.Id && po.OrganizationId == organization.Id
                )
            );
    ...
    [Theory, BitAutoData]
    public async Task InitiateDeleteAsync_ThrowsBadRequestException_WhenProviderNameIsEmpty(
    ...
    )
    {
        ...
        await Assert.ThrowsAsync<BadRequestException>(
            () => sutProvider.Sut.InitiateDeleteAsync(provider, providerAdminEmail)
        );
    }

    [Theory, BitAutoData]
    public async Task InitiateDeleteAsync_ThrowsBadRequestException_WhenProviderAdminNotFound(
        Provider provider,
        SutProvider<ProviderService> sutProvider
    )
    {
    ...
    [Theory, BitAutoData]
    public async Task InitiateDeleteAsync_SendsInitiateDeleteProviderEmail(
        Provider provider,
        User providerAdmin,
        ProviderUser providerUser,
        SutProvider<ProviderService> sutProvider
    )
    {
        ...
        await mailService
            .Received()
            .SendInitiateDeletProviderEmailAsync(providerAdminEmail, provider, Arg.Any<string>());
    }

    [Theory, BitAutoData]
    public async Task DeleteAsync_ThrowsBadRequestException_WhenInvalidToken(
        Provider provider,
        string invalidToken,
        SutProvider<ProviderService> sutProvider
    )
    {
        var providerDeleteTokenDataFactory = sutProvider.GetDependency<
            IDataProtectorTokenFactory<ProviderDeleteTokenable>
        >();
        providerDeleteTokenDataFactory
            .TryUnprotect(invalidToken, out Arg.Any<ProviderDeleteTokenable>())
            .Returns(false);
    ...
    [Theory, BitAutoData]
    public async Task DeleteAsync_ThrowsBadRequestException_WhenInvalidTokenData(
        Provider provider,
        string validToken,
        SutProvider<ProviderService> sutProvider
    )
    ...
    private static SubscriptionUpdateOptions SubscriptionUpdateRequest(
