...
public class RemoveOrganizationFromProviderCommandTests
{
    ...
    )
    {
        var exception = await Assert.ThrowsAsync<BadRequestException>(
            () => sutProvider.Sut.RemoveOrganizationFromProvider(null, null, null)
        );
    ...
    {
        var exception = await Assert.ThrowsAsync<BadRequestException>(
            () => sutProvider.Sut.RemoveOrganizationFromProvider(provider, null, null)
        );
    ...
    {
        var exception = await Assert.ThrowsAsync<BadRequestException>(
            () => sutProvider.Sut.RemoveOrganizationFromProvider(provider, providerOrganization, null)
        );
    ...
    {
        var exception = await Assert.ThrowsAsync<BadRequestException>(
            () => sutProvider.Sut.RemoveOrganizationFromProvider(provider, providerOrganization, organization)
        );
    ...
    )
    {
        ...
        sutProvider
            ...
            .HasConfirmedOwnersExceptAsync(providerOrganization.OrganizationId, [], includeProvider: false)
            .Returns(false);
    ...
    )
    {
        ...
        organizationRepository
            .GetOwnerEmailAddressesById(organization.Id)
        ...
        await organizationRepository
            .Received(1)
            .ReplaceAsync(Arg.Is<Organization>(org => org.BillingEmail == "a@example.com"));
    ...
    )
    {
        ...
        stripeAdapter
            ...
            .Returns(new Subscription { Id = "subscription_id" });
